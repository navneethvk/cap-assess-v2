...
exports.panelCreateUser = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
    ...
    const email = ((data === null || data === void 0 ? void 0 : data.email) || "").toString().trim();
    const password = ((data === null || data === void 0 ? void 0 : data.password) || "").toString();
    const newRole = ((data === null || data === void 0 ? void 0 : data.role) || "Pending").toString();
    const username = ((data === null || data === void 0 ? void 0 : data.username) || "").toString().trim();
    if (!email || !password) {
...
function formatIstTimestampForFilename(date) {
    ...
    const year = (_a = map.get("year")) !== null && _a !== void 0 ? _a : "0000";
    const month = (_b = map.get("month")) !== null && _b !== void 0 ? _b : "00";
    const day = (_c = map.get("day")) !== null && _c !== void 0 ? _c : "00";
    const hour = ((_d = map.get("hour")) !== null && _d !== void 0 ? _d : "00").replace(/[^0-9]/g, "");
    const minute = (_e = map.get("minute")) !== null && _e !== void 0 ? _e : "00";
    const second = (_f = map.get("second")) !== null && _f !== void 0 ? _f : "00";
...
async function fetchVisitRows() {
    ...
    while (true) {
        ...
        lastDocumentId = (_b = (_a = snapshot.docs[snapshot.docs.length - 1]) === null || _a === void 0 ? void 0 : _a.id) !== null && _b !== void 0 ? _b : null;
...
function getStorageTarget() {
    var _a, _b, _c;
    const bucketName = (_a = functions.config().exports) === null || _a === void 0 ? void 0 : _a.bucket;
    const prefix = (_c = (_b = functions.config().exports) === null || _b === void 0 ? void 0 : _b.prefix) !== null && _c !== void 0 ? _c : DEFAULT_STORAGE_PREFIX;
    return {
        bucketName,
...
async function exportVisitsCsvInternal() {
    ...
    return Object.assign(Object.assign({}, uploadResult), { fileName, rowCount: rows.length });
}
exports.exportVisitsCsvNightly = functions.pubsub
    .schedule("0 3 * * *")
    .timeZone(IST_TIME_ZONE)
    .onRun(async () => {
    try {
        ...
        functions.logger.error("Visits CSV export failed", error);
        throw error;
    }
});
exports.panelExportVisitsCsv = functions.https.onCall(async (_data, context) => {
    if (!context.auth) {
    ...
    try {
        const result = await exportVisitsCsvInternal();
        functions.logger.info("Visits CSV export triggered by admin", Object.assign({ uid: context.auth.uid }, result));
        return result;
    }
    catch (error) {
        const message = error instanceof Error ? error.message : "Unknown error";
        functions.logger.error("Admin-triggered visits CSV export failed", error);
        throw new functions.https.HttpsError("internal", `Failed to export visits CSV: ${message}`);
    }
});
// Version History Cloud Function
exports.captureVersionHistory = functions.firestore
    ...
    try {
        ...
        const userName = (userData === null || userData === void 0 ? void 0 : userData.email) || (userData === null || userData === void 0 ? void 0 : userData.username) || "Unknown User";
        const events = [];
        ...
        // Find added notes
        const addedNotes = afterNotes.filter((note) => !beforeNotes.some((beforeNote) => beforeNote.id === note.id));
        // Find edited notes
        const editedNotes = afterNotes.filter((note) => {
            const beforeNote = beforeNotes.find((beforeNote) => beforeNote.id === note.id);
            return beforeNote && stripHtml(beforeNote.text || "") !== stripHtml(note.text || "");
        });
        // Find deleted notes
        const deletedNotes = beforeNotes.filter((note) => !afterNotes.some((afterNote) => afterNote.id === note.id));
        // Add note events
        ...
        for (const note of editedNotes) {
            ...
            events.push({
                type: "note_edit",
                beforeValue: stripHtml((beforeNote === null || beforeNote === void 0 ? void 0 : beforeNote.text) || ""),
                afterValue: stripHtml(note.text || ""),
                userId,
                userName,
                timestamp: admin.firestore.FieldValue.serverTimestamp(),
...
async function checkAndCreateSnapshot(visitId) {
    try {
        ...
        const allEvents = eventsSnapshot.docs.map(doc => (Object.assign({ id: doc.id }, doc.data()))).sort((a, b) => {
            var _a, _b;
            // Sort by timestamp (ascending - oldest first)
            const aTime = ((_a = a.timestamp) === null || _a === void 0 ? void 0 : _a.toDate) ? a.timestamp.toDate() : new Date(a.timestamp);
            const bTime = ((_b = b.timestamp) === null || _b === void 0 ? void 0 : _b.toDate) ? b.timestamp.toDate() : new Date(b.timestamp);
        ...
        const snapshottedEventIds = new Set();
...
function generateSnapshotSummary(events) {
    ...
    const summaryParts = [];
    if (typeCounts.agenda_edit)
        summaryParts.push(`${typeCounts.agenda_edit} agenda edit${typeCounts.agenda_edit > 1 ? 's' : ''}`);
    if (typeCounts.debrief_edit)
        summaryParts.push(`${typeCounts.debrief_edit} debrief edit${typeCounts.debrief_edit > 1 ? 's' : ''}`);
    if (typeCounts.note_add)
        summaryParts.push(`${typeCounts.note_add} note${typeCounts.note_add > 1 ? 's' : ''} added`);
    if (typeCounts.note_edit)
        summaryParts.push(`${typeCounts.note_edit} note${typeCounts.note_edit > 1 ? 's' : ''} edited`);
    if (typeCounts.note_delete)
    ...
    try {
            ...
            for (let i = 0; i < userIds.length; i += batchSize) {
                ...
                usersSnapshot.forEach(doc => {
                    ...
                    userDataMap.set(doc.id, {
                        ...
                        displayName: ((_d = (_c = data.displayName) !== null && _c !== void 0 ? _c : data.fullName) !== null && _d !== void 0 ? _d : null),
                        role: ((_e = data.role) !== null && _e !== void 0 ? _e : null)
                    });
                });
            }
        }
        if (userDataMap.size > 0) {
            userStats.forEach((userStat, uid) => {
                ...
                userStat.email = (_a = metadata.email) !== null && _a !== void 0 ? _a : userStat.email;
                userStat.username = (_b = metadata.username) !== null && _b !== void 0 ? _b : userStat.username;
                userStat.displayName = (_c = metadata.displayName) !== null && _c !== void 0 ? _c : userStat.displayName;
                if (metadata.role) {
        ...
        const perUserStatsByUid = new Map();
        ...
        const serializePerUserEntry = (entry) => ({
            uid: entry.uid,
            email: entry.email,
        ...
        const perUserStatsByUidForFirestore = Object.fromEntries(Array.from(perUserStatsByUid.entries()).map(([uid, entry]) => [
            uid,
        ...
        const perUserStatsListForFirestore = Array.from(perUserStatsByUid.entries())
            ...
            const sortKey = ((_a = label === null || label === void 0 ? void 0 : label.toLowerCase) === null || _a === void 0 ? void 0 : _a.call(label)) || uid.toLowerCase();
            return {
                sortKey,
                uid,
                label,
            ...
            .map((_a) => {
            var { sortKey } = _a, rest = __rest(_a, ["sortKey"]);
            return rest;
        });
