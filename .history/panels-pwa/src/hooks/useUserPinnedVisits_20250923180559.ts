...
export const useUserPinnedVisits = () => {
  ...
  // Use ref to access current pinnedVisits without causing re-renders
  const pinnedVisitsRef = useRef<string[]>([]);
  pinnedVisitsRef.current = pinnedVisits;
  ...
  }, [user?.uid, currentUser?.pinnedVisits]);
  ...
  const togglePin = useCallback(async (visitId: string) => {
    ...
    setIsLoading(true);
    try {
      const currentPinnedVisits = pinnedVisitsRef.current;
      const isCurrentlyPinned = currentPinnedVisits.includes(visitId);
      const newPinnedVisits = isCurrentlyPinned
        ? currentPinnedVisits.filter(id => id !== visitId)
        : [...currentPinnedVisits, visitId];
