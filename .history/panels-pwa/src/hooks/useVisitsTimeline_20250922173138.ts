...
// Custom hook that conditionally fetches admin visits
const useAdminVisits = (isAdmin: boolean) => {
  const result = useFirestoreCollection<VisitDoc>(
    'visits',
    { revalidateOnFocus: false }
  )
  
  // Return empty data for non-admin users to avoid unnecessary network requests
  if (!isAdmin) {
    return {
      data: [],
      error: null,
...
export const useVisitsTimeline = (): VisitsTimelineState => {
  ...
  // For non-admin users, we'll use a dummy hook that returns empty data
  const adminVisitsHook = useFirestoreCollection<VisitDoc>(
    'visits',
    { revalidateOnFocus: false }
  )
  
  // Only use admin visits data if user is admin, otherwise provide empty fallback
  const visitsRaw = isAdmin ? adminVisitsHook.data : []
  const visitsError = isAdmin ? adminVisitsHook.error : null
  const mutateVisitsRaw = isAdmin ? adminVisitsHook.mutate : () => Promise.resolve()
