...
export const useVisitsTimeline = (): VisitsTimelineState => {
  ...
  const { visits: emVisits, mutate: mutateEmVisits, error: emError, isLoading: emLoading } = useVisitsInRange(startOfDay, endOfDay, {
    filledByUid: user?.uid ?? null,
  })

  const allVisits = isAdmin ? adminVisits : emVisits
  const currentError = isAdmin ? adminError : emError
  const mutate = isAdmin ? mutateAdminVisits : mutateEmVisits
  const isLoading = isAdmin ? adminLoading : emLoading
