...
export const useVisitQueries = (options: UseVisitQueriesOptions = {}) => {
  ...
  if (error && !shouldShowError) {
    if (isIndexError) {
      console.log('useVisitQueries: Suppressing Firestore index error:', error.message);
    } else {
      console.log('useVisitQueries: Suppressing error during initial loading:', error);
    }
  }
  if (shouldShowError) {
    console.log('useVisitQueries: Showing error:', error);
  }
