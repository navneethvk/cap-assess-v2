...
const useAdminVisits = (isAdmin: boolean) => {
  // Use a dummy collection path for non-admin users to avoid network requests
  const collectionPath = isAdmin ? 'visits' : 'dummy-collection-that-does-not-exist'
...
export const useVisitsTimeline = (): VisitsTimelineState => {
  ...
  const { data: visitsRaw, error: visitsError, mutate: mutateVisitsRaw } = useAdminVisits(isAdmin)
