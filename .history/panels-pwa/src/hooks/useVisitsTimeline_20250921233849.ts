...
export const useVisitsTimeline = (): VisitsTimelineState => {
  ...
  const { data: visitsRaw, error: visitsError, mutate: mutateVisitsRaw } = useFirestoreCollection<VisitDoc>('visits', { revalidateOnFocus: false })
  ...
  return {
    ...
    isLoading: !allVisits && !(isAdmin ? visitsError : null),
    error: isAdmin ? visitsError : null,
