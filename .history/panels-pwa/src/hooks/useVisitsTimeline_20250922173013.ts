...
export const useVisitsTimeline = (): VisitsTimelineState => {
  ...
  // Use conditional hook pattern to avoid calling useFirestoreCollection with empty path
  const adminVisitsResult = isAdmin ? useFirestoreCollection<VisitDoc>(
    'visits',
    { revalidateOnFocus: false }
  ) : { data: [], error: null, mutate: () => Promise.resolve() }
  
  const visitsRaw = adminVisitsResult.data || []
  const visitsError = adminVisitsResult.error
  const mutateVisitsRaw = adminVisitsResult.mutate
