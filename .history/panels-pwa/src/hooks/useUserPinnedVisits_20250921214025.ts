...
export const useUserPinnedVisits = () => {
  ...
  // Fetch user data to get pinned visits
  const { data: userData, mutate: mutateUser } = useFirestoreCollection<UserDoc>('users');

  // Update local state when user data changes
  useEffect(() => {
    console.log('useUserPinnedVisits: user:', user?.uid);
    console.log('useUserPinnedVisits: userData count:', userData?.length || 0);
  ...
  // Check if a visit is pinned
  const isPinned = useCallback((visitId: string) => {
    const result = pinnedVisits.includes(visitId);
    console.log('isPinned:', visitId, '->', result, 'pinnedVisits:', pinnedVisits);
    return result;
  }, [pinnedVisits]);
