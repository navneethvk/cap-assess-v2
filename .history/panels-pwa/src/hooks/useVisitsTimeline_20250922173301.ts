...
const useAdminVisits = (isAdmin: boolean) => {
  // Use SWR's conditional fetching - pass null path when not admin
  return useFirestoreCollection<VisitDoc>(
    isAdmin ? 'visits' : null, // SWR will not fetch when path is null
