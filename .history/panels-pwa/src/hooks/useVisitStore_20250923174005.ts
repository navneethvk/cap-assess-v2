...
import useVisitStoreStore, { createWindowKey } from '@/store/visitStore';
...
export const useVisitStore = () => {
  ...
  const loadWindow = useCallback(async (startDate: Date, endDate: Date, forceRefresh = false) => {
    ...
    if (!forceRefresh && store.windows[windowKey] && store.windows[windowKey].lastFetched) {
      const lastFetchedRaw = store.windows[windowKey].lastFetched as unknown as Date | string | null
      const lastFetchedDate = lastFetchedRaw instanceof Date
        ? lastFetchedRaw
        : (typeof lastFetchedRaw === 'string' ? new Date(lastFetchedRaw) : null)

      const timeSinceFetch = lastFetchedDate ? (Date.now() - lastFetchedDate.getTime()) : Number.POSITIVE_INFINITY;
    ...
    try {
      ...
      const visits = (visitsData as DocumentWithId<VisitDoc>[]).filter(visit => {
  ...
  const getUserVisits = useCallback(() => {
    ...
    Object.values(store.windows).forEach((window: any) => {
      window.visits.forEach((visit: any) => {
