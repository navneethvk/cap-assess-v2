...
export const useVisitsTimeline = (): VisitsTimelineState => {
  ...
  // Data fetching
  const { data: allVisits, error: visitsError, mutate } = useUserVisits()
  const { data: allUsers } = useUsersForVisits()
  const ccisPath = ccisCollection()
  console.log('CCIs collection path:', ccisPath, 'Type:', typeof ccisPath)
  const { data: allCcis } = useFirestoreCollection<CCI>(ccisPath || 'ccis')
  ...
  useEffect(() => {
    ...
    const tasks = updates.map(u => updateDocument(visitsCollection(), u.id, { order: u.order }))
  ...
  return {
    ...
    isLoading: !allVisits && !visitsError,
    error: visitsError,
