...
export const captureVersionEvent = async (
  ...
  try {
    ...
    console.log('Capturing version event:', { visitId, event: versionEvent });
    
    // Temporarily disabled until Firestore rules are deployed
    // await addDocument(visitEventsCollection(visitId), versionEvent);
    console.log('Version event capture temporarily disabled - deploy Firestore rules to enable');
  } catch (error) {
