...
export const createSimpleDiff = (before: string, after: string): string => {
  ...
  if (!before) return `Added: ${truncateText(after, 100)}`;
  if (!after) return `Removed: ${truncateText(before, 100)}`;
  if (before === after) return 'No changes';
  ...
  const maxLength = Math.min(before.length, after.length, 200);
  for (let i = 0; i < maxLength; i++) {
    if (before[i] !== after[i]) {
      const start = Math.max(0, i - 20);
      const end = Math.min(maxLength, i + 20);
      const beforeSnippet = before.substring(start, end);
      const afterSnippet = after.substring(start, end);
      return `Changed: "${truncateText(beforeSnippet, 50)}" → "${truncateText(afterSnippet, 50)}"`;
    }
  }
