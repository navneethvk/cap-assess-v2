    ...
    timestamp: (() => {
      try {
        ...
        if (event.timestamp && typeof event.timestamp === 'object') {
          if ('toDate' in event.timestamp && typeof event.timestamp.toDate === 'function') {
            // Firestore Timestamp object
            date = event.timestamp.toDate();
          } else if ('seconds' in event.timestamp && 'nanoseconds' in event.timestamp) {
            // Firestore Timestamp-like object
            date = new Date(event.timestamp.seconds * 1000 + event.timestamp.nanoseconds / 1000000);
          } else if (event.timestamp._seconds && event.timestamp._nanoseconds) {
            // Alternative Firestore timestamp format
            date = new Date(event.timestamp._seconds * 1000 + event.timestamp._nanoseconds / 1000000);
          } else {
