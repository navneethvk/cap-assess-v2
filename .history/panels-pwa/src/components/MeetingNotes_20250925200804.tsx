...
const MeetingNotes: React.FC = () => {
  ...
  // Check if there are changes to discard (current vs original data)
  const hasChangesToDiscard = React.useMemo(() => {
    if (!visit || !originalData) return false
    return (
      visit.agenda !== originalData.agenda ||
      visit.debrief !== originalData.debrief ||
      editedDate?.getTime() !== (originalData.date ? ((originalData.date as any)?.toDate ? (originalData.date as any).toDate() : new Date(originalData.date as any)) : null)?.getTime() ||
      editedCciId !== originalData.cci_id
    )
  }, [visit, originalData, editedDate, editedCciId])
  ...
  const performAutosave = useCallback(async () => {
    console.log('performAutosave called:', { visit: !!visit, canEdit, isAutosaving })
    if (!visit || !canEdit || isAutosaving) {
      console.log('performAutosave early return:', { visit: !!visit, canEdit, isAutosaving })
      return
    }

    try {
      setIsAutosaving(true)
      // Use current visit data for autosave
      const agenda = visit.agenda || ''
      const debrief = visit.debrief || ''

      // Check if debrief has content to auto-complete status
      const debriefText = debrief.replace(/<[^>]*>/g, '').trim() // Strip HTML tags
  ...
  const handleDataChangeWithAutosave = useCallback(() => {
    console.log('handleDataChangeWithAutosave called, canEdit:', canEdit)
  ...
  useEffect(() => {
    ...
    setSlots({
      ...
      customCenter: (
        <TextSelect
          ...
          size="md"
          className="w-full min-w-0 max-w-[200px] sm:max-w-[240px]"
          disabled={!canEdit}
        />
      ),
      customRight: (
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <Button 
            ...
            disabled={!hasChangesToDiscard || !canEdit}
          >
            <X className="h-4 w-4" />
          </Button>
    ...
    return () => clearSlots()
  }, [visit, editedDate, editedCciId, institutionOptions, canEdit, lastAutosave, hasUnsavedChanges, hasChangesToDiscard, visitEdit.isSaving])
