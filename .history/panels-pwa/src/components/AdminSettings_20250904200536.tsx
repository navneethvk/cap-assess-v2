...
import { Users, Building2, UserPlus, Sparkles, Save, AlertTriangle, ChevronDown, Search as SearchIcon, X, Pencil, Table as TableIcon } from 'lucide-react';
...
const UserSettings: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'users'|'ccis'|'assignments'|'reports'>('users');
  const { data: allVisitsRaw, isLoading: loadingVisits, error: visitsError } = useFirestoreCollection<VisitDoc>(visitsCollection());
  ...
  const completedVisits = useMemo(() => {
    const list = (allVisitsRaw || []).filter(v => {
      const text = (v.debrief || '').trim()
                  ...
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <CardTitle className="flex items-center gap-2">
                        <TableIcon className="h-5 w-5" /> Weekly Visits by EM
                      </CardTitle>
                      <CardDescription>Counts per Monday–Saturday week. A visit is complete when Debrief is not blank.</CardDescription>
                    </div>
                  </div>
                </CardHeader>
                <CardContent>
                  {loadingVisits ? (
                    <div className="py-8 text-center text-muted-foreground">Loading visits…</div>
                  ) : visitsError ? (
                    <div className="py-8 text-center text-destructive">Failed to load visits.</div>
                  ) : (
                    <PivotWeeklyByEM allUsers={allUsers || []} visits={allVisitsRaw || []} />
                  )}
                </CardContent>
              </Card>
            </TabsContent>
...
const PivotWeeklyByEM: React.FC<{ allUsers: UserProfile[]; visits: VisitDoc[] }> = ({ allUsers, visits }) => {
  const emUsers = useMemo(() => allUsers.filter(u => u.role === 'EM'), [allUsers])
  const emByUid = useMemo(() => new Map(emUsers.map(u => [u.uid, u])), [emUsers])

  // Build weeks from all visit dates (Mon-Sat) and counts per EM
  const rows = useMemo(() => {
    // helper: to Date
    const toDate = (val: any): Date => val?.toDate ? val.toDate() : new Date(val)
    // group visits by weekKey and em
  ...
  if (emUsers.length === 0) {
    return <div className="text-sm text-muted-foreground">No EM users found.</div>
  }

  if (!rows || rows.weeks.length === 0) {
    return <div className="text-sm text-muted-foreground">No completed visits found.</div>
  }
