...
const NotesView: React.FC = () => {
  ...
  const { pinnedNotes, regularNotes } = useMemo(() => {
    ...
    if (!allVisits || allVisits.length === 0) {
      console.log('NotesView: No visits data available');
    ...
    const items = allVisits.filter(v => {
      ...
      // Temporarily disable date range filtering to test
    ...
    console.log('NotesView: filtered items sample:', items.slice(0, 3).map(v => ({ id: v.id, cci_name: v.cci_name, date: v.date })));
    ...
    console.log('NotesView: pinned sample:', pinned.slice(0, 2).map(v => ({ id: v.id, cci_name: v.cci_name })));
    console.log('NotesView: regular sample:', regular.slice(0, 2).map(v => ({ id: v.id, cci_name: v.cci_name })));
  ...
  const NoteCard: React.FC<{ visit: VisitDoc }> = ({ visit }) => {
    ...
    console.log('NoteCard: rendering visit:', visit.id, 'cci_name:', visit.cci_name, 'date:', date);
    return (
      <PinButton
        isPinned={isPinned}
  ...
  console.log('NotesView: Rendering - hasNotes:', hasNotes);
  console.log('NotesView: Rendering - allNotes.length:', allNotes.length);
  console.log('NotesView: Rendering - pinnedNotes.length:', pinnedNotes.length);
  console.log('NotesView: Rendering - regularNotes.length:', regularNotes.length);
