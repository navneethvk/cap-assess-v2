...
import { UserPlus, Building2, Save, Search as SearchIcon, X } from 'lucide-react';
...
const AssignUsers: React.FC = () => {
  const { data: users, isLoading: loadingUsers, error: usersError } = useFirestoreCollection<User>(usersCollection(), {
    revalidateOnFocus: false,
    revalidateIfStale: false
  });
  const { data: ccis, isLoading: loadingCcis, error: ccisError } = useFirestoreCollection<CCI>(ccisCollection(), {
    revalidateOnFocus: false,
    revalidateIfStale: false
  });
  const { data: cciUserLinks, isLoading: loadingLinks, error: linksError, mutate: mutateLinks } = useFirestoreCollection<CciUserLink>(cciUserLinksCollection(), {
    revalidateOnFocus: false,
    revalidateIfStale: false
  });
  ...
  // Debug logging
  useEffect(() => {
    console.log('AssignUsers: Component mounted');
    console.log('AssignUsers: Users loading:', loadingUsers, 'count:', users?.length || 0, 'error:', usersError);
    console.log('AssignUsers: CCIs loading:', loadingCcis, 'count:', ccis?.length || 0, 'error:', ccisError);
    console.log('AssignUsers: Links loading:', loadingLinks, 'count:', cciUserLinks?.length || 0, 'error:', linksError);
  }, [loadingUsers, users, usersError, loadingCcis, ccis, ccisError, loadingLinks, cciUserLinks, linksError]);
  ...
  const hasErrors = usersError || ccisError || linksError;
  const isLoading = loadingUsers || loadingCcis || loadingLinks;

  // Show error state if any collection failed to load
  if (hasErrors) {
    console.error('AssignUsers: Errors detected:', { usersError, ccisError, linksError });
    return (
      <Card className="shadow-xl">
        <CardHeader>
          <CardTitle className="text-destructive">Error Loading Assignment Data</CardTitle>
          <CardDescription>
            There were errors loading the required data. Please try refreshing the page.
          </CardDescription>
        </CardHeader>
          ...
          {usersError && (
            <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-lg">
              <p className="text-sm text-destructive">
                <strong>Users Error:</strong> {usersError.message || 'Unknown error occurred'}
              </p>
            </div>
          )}
          {ccisError && (
            <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-lg">
              <p className="text-sm text-destructive">
                <strong>CCIs Error:</strong> {ccisError.message || 'Unknown error occurred'}
              </p>
            </div>
          )}
          {linksError && (
            <div className="p-4 bg-destructive/10 border border-destructive/20 rounded-lg">
              <p className="text-sm text-destructive">
                <strong>User Links Error:</strong> {linksError.message || 'Unknown error occurred'}
              </p>
            </div>
          )}
          <Button 
  ...
  // Show loading state
  if (isLoading) {
    console.log('AssignUsers: Rendering loading skeleton');
    return (
  ...
  console.log('AssignUsers: Rendering main component with', {
    usersCount: users?.length || 0,
    ccisCount: ccis?.length || 0,
    linksCount: cciUserLinks?.length || 0
  });
  ...
  const saveUserAssignments = async (userId: string) => {
    ...
    try {
      ...
      await setDocument(cciUserLinksCollection(), userId, {
        user_id: userId,
        cci_id: assignedCcis,
  ...
  const saveAllAssignments = async () => {
    ...
    try {
      ...
      const savePromises = toSave.map((userId) => 
        setDocument(cciUserLinksCollection(), userId, {
          user_id: userId,
                ...
                <div className="p-4 pt-3">
                  ...
                  >
                    ...
                    <div className="max-h-[40vh] overflow-y-auto space-y-2">
                      {filteredCcis
                        .filter(cci => !(userAssignments[selectedUser.uid] || []).includes(cci.id))
                        .map(cci => (
                          <div
                            key={cci.id}
                            draggable
