...
import { GripVertical, Save as SaveIcon, X as XIcon } from 'lucide-react'
...
const TimelineCard: React.FC<{ 
  ...
  const isDirty = (agenda !== baseAgenda) || (debrief !== baseDebrief)
  ...
  return (
    ...
    >
      ...
      <Card className={`relative mb-4 transition-all duration-200 bg-white dark:bg-[hsl(var(--card))] border border-blue-100/70 dark:border-foreground/20 rounded-none shadow-md ${anyDragging ? 'max-h-16 overflow-hidden' : ''}`}>
        ...
        {/* Action icons: Save (top-right), Discard (bottom-right) */}
        {expanded && !anyDragging && isDirty && (
          <>
            <button
              type="button"
              ...
              onClick={async (e) => { e.stopPropagation(); await save({ agenda, debrief }); setBaseAgenda(agenda); setBaseDebrief(debrief); }}
            >
            ...
            <button
              ...
              onClick={(e) => { e.stopPropagation(); setAgenda(baseAgenda); setDebrief(baseDebrief); }}
            >
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            <div
              ...
              onMouseDown={(e) => { e.stopPropagation(); e.preventDefault(); (window as any)._agendaLP = setTimeout(() => { (window as any)._agendaLP = null; }, 500); }}
              onMouseUp={(e) => { e.stopPropagation(); if ((window as any)._agendaLP) { clearTimeout((window as any)._agendaLP); } }}
              onDoubleClick={(e) => { e.stopPropagation(); }}
              contentEditable
              suppressContentEditableWarning
              onInput={(e) => setAgenda((e.target as HTMLDivElement).innerText)}
            >
              {agenda || '—'}
            </div>
          </div>
          ...
          <div>
            ...
            <div
              className="w-full min-h-[3rem] border rounded-md p-3 text-sm bg-slate-50"
              onMouseDown={(e) => { e.stopPropagation(); e.preventDefault(); (window as any)._debriefLP = setTimeout(() => { (window as any)._debriefLP = null; }, 500); }}
              onMouseUp={(e) => { e.stopPropagation(); if ((window as any)._debriefLP) { clearTimeout((window as any)._debriefLP); } }}
              onDoubleClick={(e) => { e.stopPropagation(); }}
              contentEditable
              suppressContentEditableWarning
              onInput={(e) => setDebrief((e.target as HTMLDivElement).innerText)}
            >
              {debrief || '—'}
            </div>
          </div>
