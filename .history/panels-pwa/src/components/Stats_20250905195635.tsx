            ...
            {isLoading ? (
              ...
              <div className="overflow-auto -mx-4 sm:mx-0">
                <div className="min-w-full px-2 sm:px-0">
                  {/* Month Calendar Style Grid */}
                  <div className="border border-gray-200 rounded-lg overflow-hidden">
                    {/* Header Row - Week Column + Participants */}
                    <div className="grid grid-cols-7 bg-gray-100 border-b border-gray-200">
                      <div className="p-1 md:p-2 text-center text-xs md:text-sm font-medium text-gray-600 border-r border-gray-200">
                        <span className="hidden sm:inline">Week</span>
                        <span className="sm:hidden">W</span>
                      </div>
                      {participants.slice(0, 6).map(p => (
                        <div key={p.uid} className="p-1 md:p-2 text-center text-xs md:text-sm font-medium text-gray-600 border-r border-gray-200 last:border-r-0">
                    ...
                    {/* Totals Row */}
                    <div className="grid grid-cols-7 bg-gray-100 border-b border-gray-200">
                      <div className="p-1 md:p-2 text-center text-xs md:text-sm font-bold text-gray-700 border-r border-gray-200">
                      ...
                      {participants.slice(0, 6).map(p => {
                        ...
                        return (
                          <div key={p.uid} className="p-1 md:p-2 text-center text-xs md:text-sm font-bold text-gray-700 border-r border-gray-200 last:border-r-0">
                            {colTotal}
                          </div>
                        )
                      })}
                    ...
                    {rows.weeks.map(([key, meta]) => {
                      ...
                      return (
                        <div key={key} className="grid grid-cols-7 border-b border-gray-200 last:border-b-0">
                          <div className="p-1 md:p-2 text-center text-xs md:text-sm font-medium bg-white border-r border-gray-200">
                          ...
                          {participants.slice(0, 6).map(p => (
                            <div key={p.uid} className="p-1 md:p-2 text-center text-xs md:text-sm bg-white border-r border-gray-200 last:border-r-0 hover:bg-gray-50">
                              {row.get(p.uid) || 0}
                            </div>
                          ))}
                        </div>
                  ...
                  {/* Show more participants if there are more than 6 */}
                  {participants.length > 6 && (
                    <div className="mt-4 text-center text-xs text-gray-500">
                      +{participants.length - 6} more participants (scroll horizontally to see all)
                    </div>
                  )}
                </div>
              </div>
