...
const VersionHistoryPopup: React.FC<VersionHistoryPopupProps> = ({
  ...
  const validEvents = allEvents?.filter(event => {
    try {
      ...
      console.warn('Invalid event timestamp:', event.timestamp, error);
      return false;
    }
  }) || [];
  ...
  const sortedEvents = validEvents?.sort((a, b) => {
    try {
      const aTime = (a.timestamp && typeof a.timestamp === 'object' && 'toDate' in a.timestamp && typeof a.timestamp.toDate === 'function') 
        ? a.timestamp.toDate() : new Date(a.timestamp);
      const bTime = (b.timestamp && typeof b.timestamp === 'object' && 'toDate' in b.timestamp && typeof b.timestamp.toDate === 'function') 
  ...
  const formatTimestamp = (timestamp: any) => {
    try {
      ...
      // Use toLocaleString for robust formatting without date-fns
