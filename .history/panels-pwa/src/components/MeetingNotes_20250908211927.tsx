...
const MeetingNotes: React.FC = () => {
  ...
  useEffect(() => {
    const checkAndUpdateStatus = async () => {
      if (visit && visit.status !== 'Complete') {
        const hasDebrief = visit.debrief && visit.debrief.trim().length > 0
        const hasPersonMet = visit.personMet && visit.personMet.length > 0
        
        if (hasDebrief || hasPersonMet) {
          try {
            await updateDocument(visitsCollection(), visit.id, { status: 'Complete' })
            mutate() // Refresh data
          } catch (error) {
            console.error('Error auto-completing status:', error)
          }
        }
      }
    }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const shouldAutoComplete = debriefText.length > 0 && visit.status !== 'Complete'
  ...
  return (
    <div className="min-h-screen bg-background pb-20">
      <div className="w-full max-w-4xl lg:max-w-6xl xl:max-w-7xl mx-auto px-3 sm:px-4 lg:px-6 xl:px-8 py-6 sm:py-10 lg:py-12 xl:py-16">
        {/* Header */}
