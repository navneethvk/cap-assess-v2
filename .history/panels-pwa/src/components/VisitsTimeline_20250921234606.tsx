      ...
      {displayVisits.length === 0 ? (
        ...
        <>
          <div className="text-center py-2 text-green-600">
            Found {displayVisits.length} visits for this date
          </div>
                ...
                <div className={`${left ? '' : 'md:pl-10'} max-w-md md:ml-0`}>
                      {left && (
                        <div className="md:mr-auto relative z-20" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                          {console.log('VisitsTimeline - About to render TimelineCard for visit (LEFT):', v.id, 'left:', left)}
                          <TimelineCard
                            visit={v}
                            onUpdated={mutate}
                      ...
                      {!left && (
                        <div className="md:mr-auto relative z-20" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                          {console.log('VisitsTimeline - About to render TimelineCard for visit (RIGHT):', v.id, 'left:', left)}
                          <TimelineCard
                            visit={v}
                            onUpdated={mutate}
                            onDragStart={handleDragStart}
