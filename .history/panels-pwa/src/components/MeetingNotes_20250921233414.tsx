...
const MeetingNotes: React.FC = () => {
  ...
  // Fetch visit data
  const { data: visits, isLoading, error, mutate } = useFirestoreCollection<VisitDoc>('visits')
  const { data: users } = useUsersForVisits()
  const { data: ccis } = useFirestoreCollection<{ id: string; name: string; city: string }>('ccis')
  ...
  useEffect(() => {
    ...
    setSlots({
      customLeft: (
        <div className="flex items-center gap-1 sm:gap-2">
          <Button onClick={() => navigate(-1)} variant="ghost" size="sm" className="h-7 w-7 p-0 flex-shrink-0" title="Back">
          ...
          {isEditing ? (
            ...
            <span className="text-xs text-gray-600 truncate max-w-[100px] sm:max-w-none">
      ...
      customCenter: isEditing ? (
        <select
          className="text-xs sm:text-sm h-7 px-1 sm:px-2 border rounded-md bg-background w-full min-w-0 max-w-[200px] sm:max-w-[300px]"
          value={editedCciId || ''}
          onChange={(e) => setEditedCciId(e.target.value)}
        >
          <option value="">Select Institution</option>
          {organizedCcis.userCcis.map((c: any) => (
            <option key={c.id} value={c.id}>{c.name || c.cci_name}</option>
          ))}
          {organizedCcis.userCcis.length > 0 && organizedCcis.otherCcis.length > 0 && (
            <option disabled>────────────</option>
          )}
          {organizedCcis.otherCcis.map((c: any) => (
            <option key={c.id} value={c.id}>{c.name || c.cci_name}</option>
          ))}
        </select>
      ) : (
    ...
    return () => clearSlots()
  }, [visit, isEditing, editedDate, editedCciId, ccis, setSlots, clearSlots, navigate])
      ...
      if (visit.id === 'new') {
        ...
        const newVisitData = {
          ...
          cci_name: editedCciId && ccis ? (() => {
            ...
            return (cci as any)?.name || (cci as any)?.cci_name || ''
          })() : '',
          filledByUid: '', // Will be set by the backend
          filledBy: 'EM',
          status: shouldAutoComplete ? 'Complete' : 'Scheduled',
          notes: []
        }

        // For new visits, we need to create the document first
        // This would typically be done with addDocument, but since we're using updateDocument,
        // we'll need to generate a new ID
        const newId = `visit_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`
        await updateDocument('visits', newId, newVisitData)
        ...
        await updateDocument('visits', visit.id, {
          agenda,
          debrief,
