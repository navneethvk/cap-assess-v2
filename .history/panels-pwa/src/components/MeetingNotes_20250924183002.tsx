...
const MeetingNotes: React.FC = () => {
  ...
  const { getCciOptions, getFilledByStatus } = visitEdit
  
  // Get organized CCI options from centralized logic
  const organizedCcis = getCciOptions()
  
  // Ensure organizedCcis has the correct structure
  const safeOrganizedCcis = Array.isArray(organizedCcis) 
    ? { userCcis: [], otherCcis: [] }
    : organizedCcis
  
  const institutionOptions = React.useMemo(() => {
    if (!safeOrganizedCcis) return []
    ...
    return [
      ...(safeOrganizedCcis.userCcis || []).map((c: CCIDoc) => ({
        label: formatLabel(c),
        value: c.id,
        icon: Building2,
        iconColor: 'text-primary'
      })),
      ...(safeOrganizedCcis.otherCcis || []).map((c: CCIDoc) => ({
        label: formatLabel(c),
        value: c.id,
        icon: Building2,
  ...
  const defaultInstitutionOption = React.useMemo(() => {
    if (!safeOrganizedCcis) return undefined
    const first = safeOrganizedCcis.userCcis?.[0] || safeOrganizedCcis.otherCcis?.[0]
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (visit.id === 'new') {
        ...
        // Update existing visit using centralized logic
