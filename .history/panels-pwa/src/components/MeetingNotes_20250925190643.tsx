...
const MeetingNotes: React.FC = () => {
  ...
  const [currentData, setCurrentData] = useState<VisitDoc | null>(null)   // Current edits
  ...
  // Handle data changes - will be defined after editors
  ...
  }, [editedCciId, visit, getFilledByStatus, handleDataChange])
  ...
  // Update handleDataChange to include autosave trigger
  ...
  const handleDiscard = () => {
    ...
    if (visit?.id === 'new') {
      ...
      setEditedDate(originalData.date ? (originalData.date as any)?.toDate ? (originalData.date as any).toDate() : new Date(originalData.date) : null)
