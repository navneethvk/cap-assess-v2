...
const ManageCCIs: React.FC = () => {
  ...
  const [newCci, setNewCci] = useState<Omit<CCIDoc, 'id'>>({ name: '', city: '', cohort: 'Test' });
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-xl">
        ...
        <CardContent className="p-6 flex flex-wrap gap-3">
          <PrimaryPopupButton onClick={downloadTemplate}>Download CSV template</PrimaryPopupButton>
          <input ref={fileInputRef} type="file" accept=".csv" hidden onChange={(e) => onUploadSelected(e.target.files?.[0] || null)} />
          <PrimaryPopupButton onClick={() => fileInputRef.current?.click()} disabled={isUploading}>
