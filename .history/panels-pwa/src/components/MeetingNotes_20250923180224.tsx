...
const MeetingNotes: React.FC = () => {
  ...
  // Fetch visit data - always use all visits for viewing individual notes
  // Users should be able to view any visit they have access to through NotesView
  const { visits: allVisits, error: allVisitsError, mutate: mutateAllVisits } = useAllVisits()
  ...
  // For viewing individual visits, use all visits (admin gets all, users get filtered by access)
  // For editing, we'll check permissions later
  const visits = allVisits
  const currentError = allVisitsError
  const isLoading = !allVisits && !allVisitsError
  const mutate = mutateAllVisits
