...
}> = ({ v, onUpdated, onDragStart, onDragOver, onDragLeave, onDrop, isDragging, anyDragging, isDragTarget }) => {
...
const VisitsTimeline: React.FC = () => {
  ...
  // Keep local list in sync when not in move mode
  useEffect(() => {
    if (!isInMoveMode) setLocalVisits(visits)
  }, [visits, isInMoveMode])
  ...
  const exitMoveMode = useCallback(async () => {
    ...
    <div className="mt-4">
      {isInMoveMode && (
        <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between">
          ...
          <Button size="sm" variant="outline" onClick={exitMoveMode} className="border-blue-300 text-blue-700 hover:bg-blue-100">Done</Button>
        </div>
      )}
