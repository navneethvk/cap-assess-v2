...
const NotesView: React.FC = () => {
  ...
  // Centralized, windowed fetching for notes view
  const { loadNotesData, getVisitsInRange } = useVisitStore();
  useEffect(() => {
    const from = range?.from || new Date(new Date().getFullYear(), new Date().getMonth(), 1);
    const to = range?.to || new Date();
    loadNotesData(from, to);
  }, [range?.from, range?.to, loadNotesData]);

  // Debug logging (disabled)
  // console.log('NotesView: allVisits count:', allVisits?.length || 0);

  // Materialized visits list only within the selected range
  const allVisits = useMemo(() => {
    const from = range?.from || new Date(2000,0,1);
    const to = range?.to || new Date(2100,0,1);
    return getVisitsInRange(from, to) as VisitDoc[];
  }, [range?.from, range?.to, getVisitsInRange]);
