...
const VersionHistoryPopup: React.FC<VersionHistoryPopupProps> = ({
  ...
  const { data: snapshots, isLoading: snapshotsLoading } = useFirestoreCollection<Snapshot>(
    ...
    {
      queryConstraints: [orderBy('version', 'desc')],
    }
  );

  // Fetch recent events (not yet in snapshots)
  // Note: For existing events without isSnapshot field, we'll fetch all and filter client-side
  ...
  // Filter recent events (those not in snapshots or without isSnapshot field)
  const recentEvents = allEvents?.filter(event => 
    event.isSnapshot === false || event.isSnapshot === undefined
  ) || [];
  ...
  }, [isOpen, visitId, snapshots, allEvents, recentEvents, snapshotsLoading, eventsLoading, hasData]);
                      ...
                      <div key={snapshot.id} className="border rounded-lg">
                        ...
                        {expandedSnapshot === snapshot.id && (
                          <div className="border-t p-4 bg-gray-50">
                            <div className="space-y-2">
                              {snapshot.events.map((event, index) => renderEvent(event, index))}
                            </div>
                          </div>
                        )}
                      </div>
