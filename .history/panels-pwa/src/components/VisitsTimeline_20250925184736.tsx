...
import { useSharedVisitsListener } from '@/hooks/useSharedVisitsListener'
...
const TimelineCard: React.FC<{ 
  ...
  const canEdit = canEditVisit(v.id, v.filledByUid)
  ...
  const handleStatusChange = async (newStatus: string) => {
    ...
    await save({ status: newStatus as any })
  }
        ...
        >
          ...
          {!anyDragging && canEdit && (
            <div className="flex items-center justify-center md:justify-center gap-2 mb-1" onClick={(e) => e.stopPropagation()}>
              ...
              />
              <QualityDropdown
                value={(v.quality || 'none') as any}
                onChange={handleQualityChange}
                className="text-xs"
              />
              <PersonMetDropdown
                value={(v.personMet || 'none') as any}
                onChange={handlePersonMetChange}
                className="text-xs"
              />
              <StatusDropdown
                value={(v.status || 'Scheduled') as any}
...
const VisitsTimeline: React.FC = () => {
  ...
  // Admin status is now managed by permissions store
  ...
  // Check if user is admin and update centralized permissions
  useEffect(() => {
    const checkAdminStatus = async () => {
      console.log('VisitsTimeline: checkAdminStatus called with user:', user);
      if (user) {
        try {
          const idTokenResult = await user.getIdTokenResult(true);
          const role = idTokenResult.claims.role as string;
          
          console.log('VisitsTimeline: Setting user permissions:', {
            uid: user.uid,
            role: role,
            claims: idTokenResult.claims
          });
          
          // Update centralized permission system
          setUser(user.uid, role, []); // TODO: Get assigned CCIs from user data
        } catch (err) {
          console.error('Error checking admin status:', err);
          setUser(user.uid, null, []);
        }
      } else {
        console.log('VisitsTimeline: No user, clearing permissions');
        setUser(null, null, []);
      }
    };
  ...
  // Use shared listener cache for efficient data fetching
  const { data: allVisits, error: visitsError, refresh: mutate } = useSharedVisitsListener({
    dateRange: { start, end },
    enabled: true
  })
  ...
  const visits = useMemo(() => {
    ...
    // Data is already filtered by date range in the shared listener
    // Just sort by order (existing logic)
    const sorted = list.sort((a, b) => {
      const orderA = (a as any).order ?? (a.createdAt instanceof Timestamp ? a.createdAt.toDate().getTime() : a.createdAt?.getTime?.() ?? 0)
      const orderB = (b as any).order ?? (b.createdAt instanceof Timestamp ? b.createdAt.toDate().getTime() : b.createdAt?.getTime?.() ?? 0)
      return orderA - orderB
    })
    
    return sorted
  }, [allVisits, visitsError])
            ...
            return (
              <div key={v.id} className="relative grid grid-cols-2 gap-6 md:gap-12 lg:gap-16 xl:gap-20 items-start">
                ...
                <div className={`${left ? 'pr-10' : ''}`}>
                  {left && (
                    <div className="md:ml-auto max-w-md relative z-20" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt instanceof Timestamp ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                      <TimelineCard
                        v={v}
                        onUpdated={mutate}
                ...
                <div className={`${left ? '' : 'md:pl-10'} max-w-md md:ml-0`}>
                  {!left && (
                    <div className="md:mr-auto relative z-20" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt instanceof Timestamp ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                      <TimelineCard
                        v={v}
                        onUpdated={mutate}
                        onDragStart={handleDragStart}
          ...
          {displayVisits.map((v) => {
            return (
              <div key={v.id} className="relative">
                ...
                <div className="relative z-20" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt instanceof Timestamp ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                  <TimelineCard
                    v={v}
                    onUpdated={mutate}
                    onDragStart={handleDragStart}
