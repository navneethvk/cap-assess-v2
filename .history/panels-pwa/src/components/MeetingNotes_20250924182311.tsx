...
const MeetingNotes: React.FC = () => {
  ...
  // Helper function to determine if user is EM for a specific CCI
  const getFilledByStatus = React.useCallback((cciId: string | null) => {
    ...
    try {
      ...
      if (visit.id === 'new') {
        ...
        const newVisitData: CreateData<VisitDoc> = {
          ...
          filledBy: getFilledByStatus(editedCciId),
