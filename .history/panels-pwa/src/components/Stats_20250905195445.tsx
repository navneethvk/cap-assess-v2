              ...
              <div className="overflow-auto -mx-4 sm:mx-0">
                <div className="min-w-full px-4 sm:px-0">
                  <Table>
                    <TableHeader>
                      <TableRow>
                        <TableHead className="sticky left-0 bg-background z-10 min-w-[100px] sm:min-w-[200px] text-[8px] sm:text-sm">
                          <span className="hidden sm:inline">Week</span>
                          <span className="sm:hidden">W</span>
                        </TableHead>
                        {participants.map(p => (
                          <TableHead key={p.uid} className="min-w-[60px] sm:min-w-[120px] text-center text-[8px] sm:text-sm">
                            <div className="break-words max-w-[50px] sm:max-w-none">
                              {p.label.length > 6 ? `${p.label.slice(0, 4)}...` : p.label}
                            </div>
                          </TableHead>
                        ))}
                        <TableHead className="whitespace-nowrap min-w-[40px] sm:min-w-[80px] text-center font-bold bg-gray-100 text-[8px] sm:text-sm">
                      ...
                      <TableRow className="bg-gray-100">
                        <TableHead className="sticky left-0 bg-gray-100 z-10 min-w-[120px] sm:min-w-[200px] font-bold text-xs sm:text-sm">
                          <span className="hidden sm:inline">Total</span>
                          <span className="sm:hidden">T</span>
                        </TableHead>
                        {participants.map(p => {
                          ...
                          return (
                            <TableHead key={p.uid} className="min-w-[80px] sm:min-w-[120px] text-center font-bold text-xs sm:text-sm">
                              {colTotal}
                            </TableHead>
                          )
                        })}
                        <TableHead className="whitespace-nowrap min-w-[60px] sm:min-w-[80px] text-center font-bold bg-gray-100 text-xs sm:text-sm">
                    ...
                    <TableBody>
                      {rows.weeks.map(([key, meta]) => {
                        ...
                        return (
                          <TableRow key={key}>
                            <TableCell className="font-medium sticky left-0 bg-background z-10 min-w-[120px] sm:min-w-[200px] text-xs sm:text-sm">
                            ...
                            {participants.map(p => (
                              <TableCell key={p.uid} className="text-center text-xs sm:text-sm">{row.get(p.uid) || 0}</TableCell>
                            ))}
                            <TableCell className="text-center font-bold bg-gray-100 text-xs sm:text-sm">{rowTotal}</TableCell>
                          </TableRow>
                        )
                      })}
                    </TableBody>
