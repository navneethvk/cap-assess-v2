...
const MonthCalendar: React.FC = () => {
  ...
  const handleScroll = useCallback(() => {
    ...
    // Calculate which month is currently in focus based on scroll position
    const monthHeight = 300; // Approximate height of one month
    const focusedMonthIndex = Math.floor(scrollTop / monthHeight);
    if (focusedMonthIndex >= 0 && focusedMonthIndex < visibleMonths.length) {
      setFocusedDate(visibleMonths[focusedMonthIndex]);
    }
  ...
  const renderMonth = (monthDate: Date) => {
    ...
    return (
      <div key={monthDate.toString()} className="mb-6">
        <div className="text-sm md:text-base font-semibold mb-2 text-center">
          {format(monthDate, 'MMMM yyyy')}
        </div>
        {/* Calendar days without headers */}
