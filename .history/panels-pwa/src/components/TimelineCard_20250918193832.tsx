...
import { VisitDoc, User, CCI } from '@/hooks/useVisitsTimeline'
...
export const TimelineCard: React.FC<TimelineCardProps> = ({
  ...
  const save = useCallback(async (fields: Partial<VisitDoc>) => {
    try { 
      await updateDocument(visitsCollection(), visit.id, fields)
      onUpdated()
  ...
  const handleSaveNote = useCallback(async (noteId: string, newText: string) => {
    const updatedNotes = notes.map(note => 
      note.id === noteId ? { ...note, text: newText } : note
    )
    await save({ notes: updatedNotes })
  }, [notes, save])
  ...
  return (
    ...
    >
      <Card className={`transition-all duration-200 ${anyDragging ? 'scale-95 shadow-lg' : 'hover:shadow-md'} ${isDragTarget ? 'ring-2 ring-blue-500' : ''}`}>
          ...
          <div className="flex items-start justify-between">
            <div className="flex-1 min-w-0">
              ...
              <div className="text-xs text-gray-500 mt-1">
                {format(visitDate, 'MMM dd, yyyy')} • {userDisplayName} • {cciCity}
              </div>
            </div>
            {anyDragging && (
        ...
        <CardContent className="pt-0">
          ...
          <div className="flex items-center justify-center md:justify-center gap-2 mb-3" onClick={(e) => e.stopPropagation()}>
            <PersonMetDropdown
              value={visit.personMet || 'none'}
          ...
          <div className="mb-3">
            <div className="flex items-center justify-between mb-1">
              <h4 className="text-xs font-medium text-gray-700">Agenda</h4>
              {!agendaEditable && (
                <Button
                  variant="ghost"
                  size="sm"
