...
import { Save, X, Bold, Italic, List, Link as LinkIcon, Image as ImageIcon, Hash, Quote, Plus, Trash2, Edit, Building2 } from 'lucide-react'
...
const CreateVisitForm: React.FC<CreateVisitFormProps> = ({
  ...
  const { organizedCcis } = useUserCciLinks()

  // Institution options for PillSelector
  const institutionOptions = React.useMemo(() => {
    if (showAllCcis) {
      // Show all CCIs when checkbox is checked
      if (!ccis) return []
      return ccis
        .sort((a, b) => {
          // First sort by city
      ...
      // Show only user's assigned CCIs when checkbox is unchecked
  ...
  }, [organizedCcis, ccis, showAllCcis])
  ...
  const emUser = user ? (users || []).find(u => u.uid === user.uid) : undefined
    ...
    <div className={`bg-background ${className}`}>
      <div className="w-full max-w-screen-2xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 py-4 sm:py-8">
        ...
        <div className="flex items-center gap-2 mb-4">
          <span className="px-2 py-0.5 rounded-full border text-xs">{user?.displayName || user?.email || 'EM'}</span>
        </div>
