...
const MeetingNotes: React.FC = () => {
  ...
  const [originalAgendaText, setOriginalAgendaText] = useState<string>('')
  const [originalDebriefText, setOriginalDebriefText] = useState<string>('')
  ...
  useEffect(() => {
    if (visits && visitId) {
      ...
      if (foundVisit) {
        ...
        // Store original text content for version history comparison
        setOriginalAgendaText(foundVisit.agenda || '')
        setOriginalDebriefText(foundVisit.debrief || '')
      }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Capture version history for agenda changes (compare text content, not HTML)
      const agendaText = agendaEditor.getText().trim();
      const originalAgendaText = visit.agenda ? agendaEditor.commands.setContent(visit.agenda) && agendaEditor.getText().trim() : '';
      if (originalAgendaText !== agendaText) {
        await captureVersionEvent(visit.id, {
          type: 'agenda_edit',
          beforeValue: originalAgendaText,
          afterValue: agendaText,
          userId: user.uid,
          userName: user.displayName || user.email || 'Unknown User',
        })
      }

      // Capture version history for debrief changes (compare text content, not HTML)
      const debriefText = debriefEditor.getText().trim();
      const originalDebriefText = visit.debrief ? debriefEditor.commands.setContent(visit.debrief) && debriefEditor.getText().trim() : '';
      if (originalDebriefText !== debriefText) {
        await captureVersionEvent(visit.id, {
          type: 'debrief_edit',
          beforeValue: originalDebriefText,
          afterValue: debriefText,
          userId: user.uid,
          userName: user.displayName || user.email || 'Unknown User',
        })
      }
  ...
  const handleDeleteNote = async (noteId: string) => {
    ...
    try {
      // Find the note to capture its content before deletion
      const noteToDelete = (visit.notes || []).find(note => note.id === noteId)
      const noteContent = noteToDelete ? stripHtmlForPreview(noteToDelete.text) : ''
      
      const updatedNotes = (visit.notes || []).filter(note => note.id !== noteId)
      await updateDocument(visitsCollection(), visit.id, { notes: updatedNotes })
      
      // Capture version history for note deletion
      if (noteContent) {
        await captureVersionEvent(visit.id, {
          type: 'note_delete',
          beforeValue: noteContent,
          afterValue: '',
          userId: user.uid,
          userName: user.displayName || user.email || 'Unknown User',
          metadata: { noteId }
