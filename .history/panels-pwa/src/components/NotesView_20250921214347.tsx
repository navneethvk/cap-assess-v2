...
const NotesView: React.FC = () => {
  ...
  const { isPinned, togglePin, isLoading: isPinning } = useUserPinnedVisits();
  ...
  const { data: visitsRaw } = useFirestoreCollection<VisitDoc>('visits', { revalidateOnFocus: false });
  ...
  const { pinnedNotes, regularNotes } = useMemo(() => {
    ...
    const pinned = sorted.filter(v => isPinned(v.id));
    const regular = sorted.filter(v => !isPinned(v.id));
