        ...
        <>
          ...
          {displayVisits.map((v, index) => {
            ...
            return (
                  <div key={v.id} className="relative mb-8">
                    ...
                    {isSingleVisit ? (
                      ...
                      <div className="flex justify-center">
                        <div className="relative z-20 max-w-md bg-white" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                          <TimelineCard
                            visit={v}
                            onUpdated={mutate}
                  ...
                  {left && (
                          <div className="md:mr-auto relative z-20 bg-white" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                      <TimelineCard
                              visit={v}
                        onUpdated={mutate}
                        onDragStart={handleDragStart}
          ...
          {displayVisits.map((v) => {
            ...
            return (
              <div key={v.id} className="relative">
                ...
                <div className="relative z-20 bg-white" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                  <TimelineCard
                      visit={v}
                    onUpdated={mutate}
                    onDragStart={handleDragStart}
