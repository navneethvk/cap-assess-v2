...
const AddVisit: React.FC<AddVisitProps> = ({ position = 'inline', className = '' }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Create a timestamp that ensures proper ordering based on position
      let createdAt: Date;
      if (position === 'top') {
        // For top position, create a timestamp slightly in the future to appear above existing visits
        createdAt = new Date(Date.now() + 1000);
      } else if (position === 'bottom') {
        // For bottom position, create a timestamp slightly in the past to appear below existing visits
        createdAt = new Date(Date.now() - 1000);
      } else {
        // For inline position, use current time
        createdAt = new Date();
      }
