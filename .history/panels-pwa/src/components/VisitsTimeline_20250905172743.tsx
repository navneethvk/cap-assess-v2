...
import { visitsCollection, usersCollection, ccisCollection } from '@/firebase/paths'
...
const VisitsTimeline: React.FC = () => {
  ...
  // Use different hooks based on admin status
  const { data: allVisits, mutate, error: visitsError } = isAdmin 
    ? useFirestoreCollection<VisitDoc>(visitsCollection(), { revalidateOnFocus: true })
    : useUserVisits<VisitDoc>(visitsCollection(), { revalidateOnFocus: true })
  
  const { data: allUsers } = useFirestoreCollection<User>(usersCollection())
