...
const VisitsTimeline: React.FC = () => {
  ...
  const visits = useMemo(() => {
    ...
    return list.filter(v => {
      ...
      // Sort by createdAt timestamp to maintain proper order
      ...
      const timeA = da?.getTime?.() ?? 0
      const timeB = db?.getTime?.() ?? 0
      
      // Sort in descending order (newest first) so top + button visits appear above
      return timeB - timeA
    })
  }, [allVisits, start, end])
