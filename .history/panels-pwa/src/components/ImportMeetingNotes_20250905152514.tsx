      ...
      <Card>
        ...
        <CardContent className="space-y-3">
          <div className="flex gap-2 flex-wrap">
            ...
            {built && (
              <PrimaryPopupButton onClick={importNow} disabled={isImporting || materialized.every(p => !p.ok)}>
        ...
        <Card>
          <CardHeader className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              ...
              <PrimaryPopupButton onClick={buildPreview}>Apply mappings</PrimaryPopupButton>
            </div>
          </CardHeader>
              ...
              <div>
                ...
                <div className="space-y-2">
                  {emTokens.map((tok) => (
                    <div key={String(tok)} className="flex items-center gap-2">
                      <div className="text-xs w-56 truncate">{tok}</div>
                      <select className="border rounded-md h-8 px-2" value={emMap[(tok||'').toLowerCase()] || ''} onChange={(e) => applyEmTokenMap(String(tok), e.target.value)}>
