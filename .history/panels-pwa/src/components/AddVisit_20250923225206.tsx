...
import { TextSelect, type TextSelectOption } from '@/components/ui'
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        visitHours: 'none', // Visit hours - none by default
      }
    ...
    <>
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div role="dialog" aria-modal="true" className="relative z-10 w-full max-w-2xl rounded-lg bg-card border border-border shadow-xl">
            <div className="p-6 border-b border-border flex items-center justify-between">
              <div className="text-lg font-semibold text-foreground">
                Add a Visit • {selectedDate.toLocaleDateString('en-US', { weekday: 'short', month: 'short', day: 'numeric' })}
              </div>
              <button className="text-sm text-muted-foreground hover:text-foreground transition-colors" onClick={() => setOpen(false)}>Close</button>
            </div>
              ...
              <div className="space-y-4">
                ...
                {availableCities.length > 0 && (
                  <div className="space-y-3">
                    <label className="text-sm block font-medium text-foreground">Filter by City</label>
                    <FilterChips
                      className="flex-wrap"
                      options={[
                        { label: 'All', value: 'All' },
                ...
                <div>
                  <label className="text-sm block mb-2 font-medium text-foreground">Select Institution</label>
                  <TextSelect
                    value={selectedCciId}
                    onChange={setSelectedCciId}
                    options={cciOptions.map(cci => ({
                      label: `${cci.name}${cci.city ? ` — ${cci.city}` : ''}`,
                      value: cci.id
                    }))}
                    placeholder={noAllocated ? 'No assigned CCIs (toggle view all?)' : 'Choose a CCI'}
                    size="md"
                    triggerClassName="border-2 rounded-xl"
                    contentClassName="border-2 rounded-xl"
                  />
                </div>
                <div className="flex justify-end pt-4">
                  <Button onClick={handleCreate} disabled={submitting || !selectedCciId} className="px-6">
