...
const MeetingNotes: React.FC = () => {
  ...
  // Fetch visit data - use appropriate data source based on admin status
  const { data: visitsRaw, error: visitsError, mutate: mutateVisitsRaw } = useFirestoreCollection<VisitDoc>(
    isAdmin ? 'visits' : '', // Only fetch all visits if user is admin
  ...
  useEffect(() => {
    if (visits && visitId) {
      if (visitId === 'new') {
        ...
        setVisit(foundVisit || null as VisitDoc | null)
        if (foundVisit) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (visit.id === 'new') {
        ...
        const newId = await addDocument('visits', newVisitData)
          ...
          <Card>
            <CardContent className="p-6">
              <div className="text-center text-red-600">
                {currentError ? `Error loading meeting notes: ${currentError.message}` : 'Meeting notes not found'}
              </div>
