...
const SideNav: React.FC<SideNavProps> = ({ open, onOpenChange }) => {
  ...
  const isActive = (path: string) => (location.pathname === path) || (path === '/timeline' && location.pathname === '/');
        ...
        >
          ...
          <div ref={drawerRef} className="absolute left-0 top-0 h-full w-72 bg-background side-drawer border-r shadow-xl flex flex-col outline-none" tabIndex={-1}>
            ...
            <nav className="flex-1 px-2 py-3 space-y-1 overflow-auto">
              <Link to="/timeline" onClick={() => onOpenChange(false)} className={`flex items-center gap-3 px-3 py-2 rounded-md ${isActive('/timeline') || isActive('/') ? 'bg-muted text-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}>
                <Home className="h-4 w-4" /> Day View
              </Link>
              <Link to="/calendar" onClick={() => onOpenChange(false)} className={`flex items-center gap-3 px-3 py-2 rounded-md ${isActive('/calendar') ? 'bg-muted text-foreground' : 'text-muted-foreground hover:bg-muted hover:text-foreground'}`}>
