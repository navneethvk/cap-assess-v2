...
import { Home, BarChart2, Palette, Settings, LogOut, StickyNote, Eye, EyeOff } from 'lucide-react';
...
const BottomNavBar: React.FC = () => {
  const { currentTheme, setTheme, getEffectiveTheme, seeAllVisits, setSeeAllVisits, getDefaultSeeAllSetting } = usePreferencesStore();
  const { user, logout } = useAuthStore();
  const { data: userCciLinks } = useUserCciLinks(user?.uid || '');
  const effectiveTheme = getEffectiveTheme();
  
  // Initialize See All setting based on user role (EM or not) - only run once
  useEffect(() => {
    if (userCciLinks && user?.uid) {
      const isEM = (userCciLinks.cci_id || []).length > 0;
      const defaultSetting = getDefaultSeeAllSetting(isEM);
      
      // Check if this is the first time the user is logging in (no stored preference)
      // We can detect this by checking if the current setting is the default (true)
      // and the expected default for this user type is different
      if (seeAllVisits === true && defaultSetting === false) {
        // User is not an EM but setting is still at default (true)
        // This means they haven't set a preference yet, so set it to their role default
        setSeeAllVisits(defaultSetting);
      } else if (seeAllVisits === false && defaultSetting === true) {
        // User is an EM but setting is false (non-EM default)
        // This means they haven't set a preference yet, so set it to their role default
        setSeeAllVisits(defaultSetting);
      }
      // If the setting already matches the expected default, leave it alone
      // This preserves user preferences
    }
  }, [userCciLinks, user?.uid, getDefaultSeeAllSetting]); // Removed seeAllVisits and setSeeAllVisits from dependencies
      ...
      <div className="flex justify-around items-center py-2 px-4 max-w-lg sm:max-w-xl mx-auto">
        ...
        <DropdownMenu>
          ...
          >
            ...
            >
              <div className="flex items-center">
                {seeAllVisits ? <Eye className="mr-2 h-4 w-4" /> : <EyeOff className="mr-2 h-4 w-4" />}
