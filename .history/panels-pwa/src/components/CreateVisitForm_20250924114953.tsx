...
const CreateVisitForm: React.FC<CreateVisitFormProps> = ({
  ...
  const allocatedIds = React.useMemo(() => {
    if (!user || !cciLinks) return []
    const link = cciLinks.find(l => l.id === user.uid)
    return link?.cci_id ?? []
  }, [cciLinks, user])
  ...
  const institutionOptions = React.useMemo(() => {
    ...
    const mapOption = (c: CCIDoc, iconColor: string) => ({
    ...
    return [
      ...userCcis.map(c => mapOption(c, 'text-primary')),
      ...otherCcis.map(c => mapOption(c, 'text-muted-foreground'))
    ]
  }, [ccis, allocatedIds])
