...
const VisitsTimeline: React.FC = () => {
  ...
  const visits = useMemo(() => {
    ...
    return list.filter(v => {
      ...
      const da: any = a.createdAt?.toDate ? a.createdAt.toDate() : a.createdAt
      const db: any = b.createdAt?.toDate ? b.createdAt.toDate() : b.createdAt
        ...
        <div>
          {visits.map(v => (
            <TimelineCard 
              ...
              onUpdated={mutate} 
              onDragStart={(e, visit) => {
                e.dataTransfer.setData('text/plain', JSON.stringify(visit));
              }}
              onDragOver={(e) => {
                e.preventDefault();
              }}
              onDrop={(e, targetVisit) => {
                e.preventDefault();
                const draggedVisit = JSON.parse(e.dataTransfer.getData('text/plain'));
                if (draggedVisit.id !== targetVisit.id) {
                  // Reorder the visits array
                  const newVisits = [...allVisits!];
                  const fromIndex = newVisits.findIndex(v => v.id === draggedVisit.id);
                  const toIndex = newVisits.findIndex(v => v.id === targetVisit.id);
                  if (fromIndex !== -1 && toIndex !== -1) {
                    const [movedItem] = newVisits.splice(fromIndex, 1);
                    newVisits.splice(toIndex, 0, movedItem);
                    mutate(newVisits);
                  }
                }
              }}
              isDragging={false} // This will be updated by the useDragDrop hook
              anyDragging={false} // This will be updated by the useDragDrop hook
            />
          ))}
        </div>
      )}
