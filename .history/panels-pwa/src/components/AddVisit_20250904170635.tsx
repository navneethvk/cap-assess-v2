...
const AddVisit: React.FC<AddVisitProps> = ({ position = 'inline', className = '', variant = 'default' }) => {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      // Create a custom order value based on position
      let order: number;
      if (position === 'top') {
  ...
  return (
    <>
      {/* Timeline + Button */}
      {variant === 'spine' ? (
        <div className={`relative flex items-center justify-center ${position === 'top' ? 'mb-8' : position === 'bottom' ? 'mt-8' : 'my-4'} ${className}`}>
          {/* Centered + button overlapping spine; its background naturally masks the spine behind */}
          <Button
            size="sm"
            variant="outline"
            onClick={() => setOpen(true)}
            className="h-10 w-10 p-0 rounded-full border-2 border-blue-300 text-blue-900 bg-white shadow-sm"
          >
