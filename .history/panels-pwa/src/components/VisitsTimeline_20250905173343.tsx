...
const VisitsTimeline: React.FC = () => {
  ...
  return (
    <div id="visits-timeline-root" className="mt-4 relative">
      ...
      >
        <div className="pointer-events-none absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-[3px] timeline-spine rounded -z-10" />
          ...
          {displayVisits.map((v, index) => {
            ...
            return (
              <div key={v.id} className="relative grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 items-start">
                ...
                {isInMoveMode && (
                  <span className="pointer-events-none absolute left-1/2 -translate-x-1/2 top-6 timeline-dot -z-10" />
                )}
                {/* Left column */}
                <div className={`hidden md:block ${left ? 'pr-10' : ''}`}>
                  {left && (
                    <div className="md:ml-auto max-w-md relative z-10" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                      <TimelineCard
                        v={v}
                        onUpdated={mutate}
                ...
                <div className={`${left ? '' : 'md:pl-10'} max-w-md md:ml-0`}>
                  {!left && (
                    <div className="md:mr-auto relative z-10" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                      <TimelineCard
                        v={v}
                        onUpdated={mutate}
                        onDragStart={handleDragStart}
                  ...
                  <div className="md:hidden relative z-10" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                    <TimelineCard
                      v={v}
                      onUpdated={mutate}
                      onDragStart={handleDragStart}
