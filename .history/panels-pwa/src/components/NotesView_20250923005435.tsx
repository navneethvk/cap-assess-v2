...
const NotesView: React.FC = () => {
  ...
  // Create stable references for the range values
  const rangeFromTime = range?.from?.getTime();
  const rangeToTime = range?.to?.getTime();
  const selectedCardIdExists = !!selectedCardId;
  const pinnedVisitsString = pinnedVisits.join(',');
  ...
  }), [rangeFromTime, rangeToTime, selectedCci, cciOptions, selectedCardIdExists, selectedCardId, pinnedVisitsString, togglePin]);
  ...
  const { pinnedNotes, regularNotes } = useMemo(() => {
    ...
    const items = allVisits.filter((v: VisitDoc) => {
    ...
    const sorted = items.sort((a: VisitDoc, b: VisitDoc) => {
    ...
    const pinned = sorted.filter((v: VisitDoc) => pinnedVisits.includes(v.id));
    const regular = sorted.filter((v: VisitDoc) => !pinnedVisits.includes(v.id));
