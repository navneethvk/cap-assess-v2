...
import { stripHtmlForPreview, truncate } from '@/utils/htmlUtils'
...
const TimelineCard: React.FC<{ 
  ...
  const handleStatusChange = async (newStatus: 'Scheduled' | 'Complete' | 'Cancelled') => {
    await save({ status: newStatus })
  }

  const handlePersonMetChange = async (newPersonMet: 'Primary PoC' | 'Project Coordinator' | 'Staff' | 'none') => {
    // Check if personMet has content to auto-complete status
    const shouldAutoComplete = newPersonMet !== 'none' && v.status !== 'Complete'
  ...
  const handleQualityChange = async (newQuality: 'Objectives Met' | 'Partially Met/Slow Pace' | 'Not Met' | 'Red Flag' | 'none') => {
    await save({ quality: newQuality })
  }

  const handleVisitHoursChange = async (newVisitHours: 'Full' | 'Half' | 'Drop-In' | 'Special' | 'none') => {
    await save({ visitHours: newVisitHours })
  }

  // Auto-complete status for existing visits with debrief content or personMet
  React.useEffect(() => {
    const checkAndUpdateStatus = async () => {
      if (v.status !== 'Complete') {
        const hasDebrief = v.debrief && v.debrief.trim().length > 0
        const hasPersonMet = v.personMet && v.personMet !== 'none'
        
        if (hasDebrief || hasPersonMet) {
          await save({ status: 'Complete' })
        }
      }
    }
    checkAndUpdateStatus()
  }, [v.debrief, v.personMet, v.status])
          ...
          <>
            <button
              ...
              onClick={async (e) => { 
                ...
                const shouldAutoComplete = debriefText.length > 0 && v.status !== 'Complete'
