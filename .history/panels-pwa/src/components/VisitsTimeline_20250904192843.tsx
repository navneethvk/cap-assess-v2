    ...
    >
      ...
      <Card className={`relative mb-4 transition-all duration-200 bg-white dark:bg-[hsl(var(--card))] border border-blue-100/70 dark:border-foreground/20 rounded-none shadow-md ${anyDragging ? 'max-h-16 overflow-hidden' : ''}`}>
        ...
        <CardContent className="space-y-6">
          ...
          {/* Notes (view-only; long-press to enable add UI) */}
          <div>
              ...
              addingNote ? (
                <div className="mt-2 space-y-2">
                  ...
                  <div className="flex gap-2 justify-end">
                    <Button size="sm" variant="ghost" onClick={() => { setAddingNote(false); setNotesEditable(false); setNoteDraft('') }}>Cancel</Button>
                    <Button size="sm" onClick={async () => { const item = { id: String(Date.now()), text: noteDraft, createdAt: new Date() as any }; await save({ notes: [...notes, item] }); setAddingNote(false); setNotesEditable(false); setNoteDraft('') }}>Add</Button>
                  </div>
                </div>
              ) : (
                <div className="mt-2">
