...
const MeetingNotes: React.FC = () => {
  ...
  useEffect(() => {
    console.log('MeetingNotes useEffect - visits:', visits?.length, 'visitId:', visitId);
    if (visits && visitId) {
      if (visitId === 'new') {
        ...
        console.log('MeetingNotes - Looking for visitId:', visitId);
        console.log('MeetingNotes - Available visit IDs:', visits?.map(v => v.id));
        const foundVisit = visits?.find(v => v.id === visitId)
        console.log('MeetingNotes - Found visit:', foundVisit ? 'YES' : 'NO');
