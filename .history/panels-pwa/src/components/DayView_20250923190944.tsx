...
const DayView: React.FC = () => {
  ...
  const openDatePicker = () => {
    ...
    input.type = 'date';
    input.value = formatDateForHTMLInput(selectedDate);
    input.style.position = 'absolute';
    input.style.opacity = '0';
    input.style.pointerEvents = 'none';

    // Position it somewhere on screen temporarily
    input.style.left = '50%';
    input.style.top = '50%';
    ...
    // Clean up after 1 second if no interaction
    setTimeout(() => {
  ...
  useEffect(() => {
    const center = (
      <div className="relative border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors min-w-[100px]">
        <input
          type="date"
          value={formatDateForHTMLInput(selectedDate)}
          onChange={handleDateChange}
          className="w-full h-full px-3 py-2 bg-transparent border-none outline-none cursor-pointer text-center font-semibold text-sm"
          style={{
            colorScheme: 'light',
