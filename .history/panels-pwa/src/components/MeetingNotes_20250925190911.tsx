...
import { ArrowLeft, Save, Edit, Bold, Italic, List, Link as LinkIcon, Image as ImageIcon, Hash, Quote, Plus, Trash2, History, Building2, RotateCcw } from 'lucide-react'
...
const MeetingNotes: React.FC = () => {
  ...
  const canEdit = visit ? canEditVisit(visit.id, visit.filledByUid) : false
  ...
  // Handle data changes with autosave - defined early to avoid hoisting issues
  const handleDataChangeWithAutosave = useCallback(() => {
    setHasUnsavedChanges(true)
    // Autosave will be triggered by the editors' onUpdate handlers
  }, [])
  ...
  useEffect(() => {
    ...
    return () => clearSlots()
  }, [visit, editedDate, editedCciId, institutionOptions, canEdit, lastAutosave, hasUnsavedChanges, visitEdit.isSaving, handleDataChangeWithAutosave])
  ...
  const performAutosave = useCallback(async () => {
    if (!visit || !agendaEditor || !debriefEditor || !canEdit || isAutosaving) return
  ...
  }, [visit, agendaEditor, debriefEditor, canEdit, isAutosaving, editedDate, editedCciId, userManuallySetStatus, visitEdit])
  ...
  }, [canEdit, performAutosave])

  // Update the early-defined handleDataChangeWithAutosave to include autosave trigger
  React.useEffect(() => {
    // Update the function to include autosave trigger
    const newHandleDataChangeWithAutosave = () => {
      setHasUnsavedChanges(true)
      triggerAutosave()
    }
    // This is a workaround for the hoisting issue
    Object.assign(handleDataChangeWithAutosave, newHandleDataChangeWithAutosave)
  }, [triggerAutosave])
