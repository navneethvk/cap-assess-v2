...
const NotesView: React.FC = () => {
  ...
  const selectAll = useCallback(() => {
    const allVisitIds = new Set([...pinnedNotes, ...regularNotes].map(v => v.id));
    setSelectedCards(allVisitIds);
  }, [pinnedNotes, regularNotes]);
  ...
  useEffect(() => {
    ...
    return () => clearSlots();
  }, [setSlots, clearSlots, dateRangePickerJSX, cciDropdownJSX, isSelectionMode, selectedCards, pinnedVisits, exitSelectionMode, handlePinSelected, handleUnpinSelected, selectAll]);
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-4">
                  {pinnedNotes.map((v: VisitDoc) => {
                    ...
                    <Card
                      key={v.id}
                      className={`cursor-pointer transition-all duration-200 hover:shadow-lg hover:scale-105 bg-white border rounded-lg overflow-hidden ${
