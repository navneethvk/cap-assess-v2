...
const NotesView: React.FC = () => {
  ...
  const notesRange = useMemo(() => {
    ...
    range?.from instanceof Date ? range.from.getTime() : defaultRangeRef.current.from.getTime(),
    range?.to instanceof Date ? range.to.getTime() : defaultRangeRef.current.to.getTime(),
  ])
  ...
  useEffect(() => {
    ...
    return () => clearSlots();
  }, [setSlots, clearSlots, cciOptions, selectedCci, range?.from, range?.to, selectedCardId, pinnedVisits, togglePin]);
