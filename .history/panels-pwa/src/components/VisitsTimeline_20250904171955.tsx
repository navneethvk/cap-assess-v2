...
const VisitsTimeline: React.FC = () => {
  ...
  return (
    <div className="mt-4 relative">
      ...
      <div
        className="absolute left-1/2 top-0 bottom-0 -translate-x-1/2 w-[3px] timeline-spine rounded"
        onMouseMove={(e) => {
          const containerRect = (e.currentTarget.parentElement as HTMLElement).getBoundingClientRect()
          const pageY = e.clientY - containerRect.top + containerRect.top + window.scrollY
          const indicator = document.getElementById('spine-add-indicator')
          if (indicator) { indicator.style.top = `${pageY - 20}px` }
        }}
        onMouseEnter={() => {
          const indicator = document.getElementById('spine-add-indicator')
