            ...
            return (
              <div key={v.id} className="relative grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 items-start">
                ...
                <div className={`${left ? '' : 'md:pl-10'} max-w-md md:ml-0`}>
                  ...
                  <div className="md:hidden relative z-20" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                    <TimelineCard
                      v={v}
                      onUpdated={mutate}
                      onDragStart={handleDragStart}
