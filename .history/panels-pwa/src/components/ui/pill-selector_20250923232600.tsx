...
export const PillSelector: React.FC<PillSelectorProps> = ({ 
  ...
  const placeholderOption = React.useMemo(
    () => options.find(opt => opt.isPlaceholder),
    [options]
  );
  const selectableOptions = React.useMemo(
    () => options.filter(opt => !opt.isPlaceholder),
    [options]
  );
  const currentOption = React.useMemo(() => {
    return options.find(opt => opt.value === value) ?? placeholderOption ?? selectableOptions[0];
  }, [options, value, placeholderOption, selectableOptions]);

  const IconComponent = currentOption?.icon;
  const isPlaceholderSelected = Boolean(currentOption?.isPlaceholder);
  ...
  const triggerText = (!currentOption || isPlaceholderSelected)
    ? effectivePlaceholder
    : currentOption?.label ?? effectivePlaceholder;
