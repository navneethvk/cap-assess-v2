...
export function DatePicker({ value, onChange, className, placeholder = "Pick a date" }: DatePickerProps) {
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      ...
      <PopoverContent className="w-64 p-3 bg-card border border-border shadow-lg backdrop-blur-none opacity-100" style={{ backgroundColor: 'hsl(var(--card))', opacity: 1 }} align="start">
            ...
            <div>
              ...
              >
                ...
                <SelectContent className="bg-card border border-border">
                  {dayOptions.map((day) => (
                    <SelectItem key={day} value={day.toString()} className="bg-card hover:bg-accent">
                      {day}
                    </SelectItem>
                  ))}
                </SelectContent>
              ...
              >
                ...
                <SelectContent className="bg-card border border-border">
                  {monthOptions.map((month) => (
                    <SelectItem key={month.value} value={month.value.toString()} className="bg-card hover:bg-accent">
              ...
              >
                ...
                <SelectContent className="bg-card border border-border">
                  {yearOptions.map((year) => (
                    <SelectItem key={year} value={year.toString()} className="bg-card hover:bg-accent">
                      {year}
                    </SelectItem>
                  ))}
                </SelectContent>
