  ...
  return (
    <div className={cn('w-full', className)}>
      <Select value={value} onValueChange={onChange} disabled={isDisabled}>
        ...
        <SelectContent 
          className={cn(
            'shadow-2xl rounded-xl overflow-hidden border border-border bg-card text-foreground',
            'max-h-[320px] overflow-y-auto backdrop-blur-md',
            contentClassName
          )}
          style={{ 
            backgroundColor: '#0f172a',
            border: '1px solid rgba(51, 65, 85, 0.85)',
            boxShadow: '0 18px 45px rgba(8, 47, 73, 0.45)',
            color: '#e2e8f0'
          }}
        >
          {isLoading ? (
