...
export const DateCarousel: React.FC = () => {
  ...
  return (
    <div className="w-full">
      ...
      <div ref={scrollerRef} className="flex overflow-x-auto space-x-2 pb-2 justify-center">
        {days.map((day) => (
          <div
            key={day.toString()}
            data-selected={isSameDay(day, selectedDate)}
            onClick={() => setSelectedDate(day)}
            className="flex-shrink-0 flex flex-col items-center justify-center w-12 h-16 rounded-lg cursor-pointer transition-colors hover:bg-muted/50 data-[selected=true]:bg-primary data-[selected=true]:text-primary-foreground data-[selected=true]:shadow-md"
          >
