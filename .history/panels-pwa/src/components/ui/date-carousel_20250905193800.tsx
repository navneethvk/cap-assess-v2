...
export const DateCarousel: React.FC = () => {
  ...
  // Scroll to make the selected date visible with minimal movement
  useEffect(() => {
        ...
        setTimeout(() => {
          ...
          if (scroller) {
            ...
            if (selectedElement) {
              ...
              // Calculate element position relative to scroller viewport
              const elementLeft = elementRect.left - scrollerRect.left;
              const elementRight = elementLeft + elementRect.width;
              const scrollerWidth = scroller.clientWidth;
              
              let scrollPosition = scroller.scrollLeft;
              
              // If element is outside the left edge, scroll to show it
              if (elementLeft < 0) {
                scrollPosition = scroller.scrollLeft + elementLeft - 10; // 10px margin
              }
              // If element is outside the right edge, scroll to show it
              else if (elementRight > scrollerWidth) {
                scrollPosition = scroller.scrollLeft + elementRight - scrollerWidth + 10; // 10px margin
              }
