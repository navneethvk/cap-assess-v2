...
const MeetingNotes: React.FC = () => {
  ...
  const { getFilledByStatus, canEditVisit, validateEditPermissions } = visitEdit
      ...
      customRight: isEditing ? (
        ...
        <div className="flex items-center gap-1 flex-shrink-0">
          ...
          <Button 
            ...
            variant="ghost" 
            className="h-7 w-7 p-0" 
            title={canEditVisit(visit) ? "Edit" : "You can only edit visits that you created"}
            disabled={!canEditVisit(visit)}
          >
  ...
  const handleSave = async () => {
    ...
    if (visit.id !== 'new' && !canEditVisit(visit)) {
