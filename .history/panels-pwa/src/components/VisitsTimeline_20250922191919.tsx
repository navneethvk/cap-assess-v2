...
const VisitsTimeline: React.FC = () => {
  ...
  if (DEBUG_VISITS_TIMELINE_COMPONENT) {
    ...
    console.log('VisitsTimeline - localVisits count:', localVisits.length)
        ...
        <>
          ...
          {displayVisits.map((v: any, index: number) => {
            ...
            return (
                  <div key={v.id} className="relative mb-8">
                    ...
                    {isSingleVisit ? (
                      ...
                      <div className="flex justify-center">
                        <div className="relative z-20 max-w-md bg-white" data-visit-card-pos data-order={(v as any).order ?? (timestampToDate(v.createdAt) || new Date()).getTime()}>
                          <TimelineCard
                            visit={v}
                            onUpdated={mutate}
                  ...
                  {left && (
                          <div className="md:mr-auto relative z-20 bg-white" data-visit-card-pos data-order={(v as any).order ?? (timestampToDate(v.createdAt) || new Date()).getTime()}>
                      <TimelineCard
                              visit={v}
                        onUpdated={mutate}
                        onDragStart={handleDragStart}
          ...
          {displayVisits.map((v: any) => {
            ...
            return (
              <div key={v.id} className="relative">
                ...
                <div className="relative z-20 bg-white" data-visit-card-pos data-order={(v as any).order ?? (timestampToDate(v.createdAt) || new Date()).getTime()}>
                  <TimelineCard
                      visit={v}
                    onUpdated={mutate}
                    onDragStart={handleDragStart}
