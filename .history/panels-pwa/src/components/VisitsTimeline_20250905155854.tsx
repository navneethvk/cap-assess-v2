...
import { GripVertical, Save as SaveIcon, X as XIcon, Pencil as PencilIcon } from 'lucide-react'
...
const TimelineCard: React.FC<{ 
  ...
  isReadOnly?: boolean;
}> = ({ v, onUpdated, onDragStart, onDragOver, onDragLeave, onDrop, isDragging, anyDragging, expanded, onToggle, onEnterMoveMode, isReadOnly = false }) => {
  ...
  return (
    <div
      className="relative"
      draggable={anyDragging && !isReadOnly}
      onDragStart={!isReadOnly ? (e) => onDragStart(e, v) : undefined}
      onDragOver={!isReadOnly ? (e) => onDragOver(e, v) : undefined}
      onDragLeave={!isReadOnly ? onDragLeave : undefined}
      onDrop={!isReadOnly ? (e) => onDrop(e, v) : undefined}
      onDragEnd={() => {}}
      onMouseDown={() => { if (!expanded && !isReadOnly) startLongPress() }}
      onMouseUp={cancelLongPress}
      onMouseLeave={cancelLongPress}
      onTouchStart={() => { if (!expanded && !isReadOnly) startLongPress() }}
      onTouchEnd={cancelLongPress}
      style={{
        opacity: isDragging ? 0.5 : 1,
        cursor: anyDragging && !isReadOnly ? 'grabbing' : isReadOnly ? 'default' : 'grab',
      }}
    >
      {/* Card with collapsed/expanded behavior — styled like dialog box */}
      <Card className={`relative mb-4 transition-all duration-200 bg-white dark:bg-[hsl(var(--card))] border border-blue-100/70 dark:border-foreground/20 rounded-none shadow-md ${anyDragging ? 'max-h-16 overflow-hidden' : ''}`}>
        <CardHeader 
        ...
        {/* Action icons: Pencil when not editing; Save/Discard when editing - only show if not read-only */}
        {!isReadOnly && expanded && !anyDragging && !(agendaEditable || debriefEditable) && (
          <button
            type="button"
            aria-label="Edit"
        ...
        <CardContent className="space-y-6">
          ...
          <div>
            ...
            {!isReadOnly && agendaEditable ? (
              ...
              <div
                ...
                onMouseDown={!isReadOnly ? agendaLP.start : undefined}
                onMouseUp={!isReadOnly ? agendaLP.cancel : undefined}
                onMouseLeave={!isReadOnly ? agendaLP.cancel : undefined}
                onTouchStart={!isReadOnly ? agendaLP.start : undefined}
                onTouchEnd={!isReadOnly ? agendaLP.cancel : undefined}
              >
                {agenda || '—'}
              </div>
            )}
          ...
          <div>
            ...
            {!isReadOnly && debriefEditable ? (
                <textarea className="w-full h-24 rounded-md border p-2 text-sm" value={debrief} onChange={(e) => setDebrief(e.target.value)} />
            ) : (
              <div
                className="w-full min-h-[3rem] border rounded-md p-3 text-sm bg-slate-50"
                onMouseDown={!isReadOnly ? debriefLP.start : undefined}
                onMouseUp={!isReadOnly ? debriefLP.cancel : undefined}
                onMouseLeave={!isReadOnly ? debriefLP.cancel : undefined}
                onTouchStart={!isReadOnly ? debriefLP.start : undefined}
                onTouchEnd={!isReadOnly ? debriefLP.cancel : undefined}
              >
                {debrief || '—'}
              </div>
            )}
          ...
          <div>
            ...
            <div
              className="space-y-2"
              onMouseDown={!isReadOnly ? notesLP.start : undefined}
              onMouseUp={!isReadOnly ? notesLP.cancel : undefined}
              onMouseLeave={!isReadOnly ? notesLP.cancel : undefined}
              onTouchStart={!isReadOnly ? notesLP.start : undefined}
              onTouchEnd={!isReadOnly ? notesLP.cancel : undefined}
            >
...
const VisitsTimeline: React.FC = () => {
  ...
  const { data: allVisits, mutate } = useFirestoreCollection<VisitDoc>(visitsCollection(), {
    queryConstraints: user && !isAdmin ? [ where('filledByUid','==', user.uid) ] : [],
  ...
  return (
    <div id="visits-timeline-root" className="mt-4 relative">
      ...
      {/* Center spine - interactive to show + on hover (wide zone with inner visible spine) */}
      <div
        className="absolute left-1/2 top-0 bottom-0 -translate-x-1/2 w-24 z-30 pointer-events-auto"
        onMouseMove={(e) => {
      ...
      >
        <div className="pointer-events-none absolute left-1/2 -translate-x-1/2 top-0 bottom-0 w-[3px] timeline-spine rounded" />
        {/* floating add indicator (inside zone to avoid flicker) */}
        <div id="spine-add-indicator" style={{ display: 'none' }} className="pointer-events-none absolute left-1/2 -translate-x-1/2 z-10">
          <button
            type="button"
          ...
          {displayVisits.map((v, index) => {
            ...
            return (
              <div key={v.id} className="relative grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-12 items-start">
                ...
                {isInMoveMode && (
                  <span className="pointer-events-none absolute left-1/2 -translate-x-1/2 top-6 timeline-dot" />
                )}
                {/* Left column */}
                <div className={`hidden md:block ${left ? 'pr-10' : ''}`}>
                  {left && (
                    <div className="md:ml-auto max-w-md" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                      <TimelineCard
                        v={v}
                        onUpdated={mutate}
                        ...
                        isReadOnly={isAdmin && v.filledByUid !== user?.uid}
                      />
                    </div>
                  )}
                </div>
                ...
                <div className={`${left ? '' : 'md:pl-10'} max-w-md md:ml-0`}>
                  {!left && (
                    <div className="md:mr-auto" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                      <TimelineCard
                        v={v}
                        onUpdated={mutate}
                        onDragStart={handleDragStart}
                  ...
                  <div className="md:hidden" data-visit-card-pos data-order={(v as any).order ?? (v.createdAt?.toDate ? v.createdAt.toDate().getTime() : v.createdAt?.getTime?.() ?? 0)}>
                    <TimelineCard
                      v={v}
                      onUpdated={mutate}
                      onDragStart={handleDragStart}
      ...
      {/* Bottom + Button centered on spine - only show for non-admin users or admin viewing their own visits */}
      {(!isAdmin || (isAdmin && displayVisits.length > 0 && displayVisits[0].filledByUid === user?.uid)) && (
        <AddVisit position="bottom" variant="spine" />
      )}
    </div>
  )
}
