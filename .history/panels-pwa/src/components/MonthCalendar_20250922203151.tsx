...
const MonthCalendar: React.FC = () => {
  ...
  const { user } = useAuthStore(); // kept for auth gating; may remove later
  ...
  // Centralized, windowed fetching
  const { loadCalendarData, getVisitsInRange } = useVisitStore();
  useEffect(() => {
  ...
  const visitsByDate = useMemo(() => {
    ...
    const inRange = getVisitsInRange(start, end) as VisitDoc[];
    ...
    while (day <= endDate) {
      for (let i = 0; i < 7; i++) {
        ...
        days.push(
          <div
            key={day.toString()}
            className={`h-full p-0.5 md:p-1 border border-gray-200 ${
        ...
        <div className="w-full max-w-none mx-0">
          <Card className="home-card">
            ...
            <CardContent className="px-0 pt-0">
              ...
              <div className="sticky top-0 z-10 bg-gray-100 border-b border-gray-200 px-0 py-1.5">
                <div className="flex items-center justify-between">
