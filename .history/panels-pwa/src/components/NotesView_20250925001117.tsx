...
import { DateRangePicker, type DateRangeValue, TextSelect } from '@/components/ui';
...
const NotesView: React.FC = () => {
  ...
  const { data: allVisits, loading: isLoading, error: notesError } = useSharedVisitsListener({
    dateRange: { start: notesRange.from, end: notesRange.to }
  });
        ...
        <DateRangePicker 
          ...
          onChange={(value) => {
            if (value && typeof value === 'object' && 'from' in value) {
              setRange(value as DateRangeValue)
            }
          }} 
        />
  ...
  const cciDropdownJSX = useMemo(() => {
    const textSelectOptions = [
      { label: 'All CCIs', value: 'all' },
      ...cciOptions.map(o => ({ label: o.name, value: o.id }))
    ];
    
    return (
      <TextSelect
        value={selectedCci}
        onChange={setSelectedCci}
        options={textSelectOptions}
        placeholder="Select CCI"
        size="sm"
      ...
      customRight: (
        <div className="flex items-center gap-2">
          <button
            ...
            className="h-6 w-6 rounded-full border border-border flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent/50 transition-colors"
  ...
  useEffect(() => {
    ...
    return () => clearSlots();
  }, [dateRangePickerJSX, cciDropdownJSX, isSelectionMode, selectionModeJSX]);
                    ...
                    >
                      <div className="p-4 h-48 flex flex-col">
                        ...
                        <div className="flex-1 overflow-hidden">
                          <p className="text-muted-foreground text-xs line-clamp-6 leading-relaxed">
                            {getNotesText(v)}
                          </p>
                        </div>
