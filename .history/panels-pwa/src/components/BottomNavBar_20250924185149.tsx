...
const BottomNavBar: React.FC = () => {
  ...
  useEffect(() => {
    if (userCciLinks && user?.uid) {
      ...
      // Only set if it hasn't been explicitly set by the user before
      // We can check if the current value is the default by comparing with the expected default
      if (seeAllVisits !== defaultSetting) {
        // If the current setting doesn't match the expected default for this user type,
        // it means the user has explicitly changed it, so we don't override it
        // This is a simple way to preserve user preferences
      }
    }
  }, [userCciLinks, user?.uid, getDefaultSeeAllSetting]);
