...
const ManageCCIs: React.FC = () => {
  ...
  // Debug logging
  useEffect(() => {
    console.log('ManageCCIs: Component mounted');
    console.log('ManageCCIs: Loading state:', isLoading);
    console.log('ManageCCIs: Error state:', error);
    console.log('ManageCCIs: Data count:', ccis?.length || 0);
  }, [isLoading, error, ccis]);
  ...
  const handleAddCci = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error('ManageCCIs: Failed to add CCI:', error);
  ...
  const handleUpdateCohort = async (id: string, cohort: string) => {
    try {
      ...
      console.error('ManageCCIs: Failed to update CCI cohort:', error);
  ...
  const handleDeleteCci = async (id: string) => {
    if (window.confirm('Are you sure you want to delete this CCI?')) {
      try {
        ...
        console.error('ManageCCIs: Failed to delete CCI:', error);
  ...
  if (isLoading) {
    console.log('ManageCCIs: Rendering loading skeleton');
    return (
  ...
  if (error) {
    console.error('ManageCCIs: Error rendering component:', error);
    return (
      <Card className="shadow-xl">
        <CardHeader>
          <CardTitle className="text-destructive">Error Loading CCIs</CardTitle>
          <CardDescription>
            There was an error loading the CCI data. Please try refreshing the page.
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  console.log('ManageCCIs: Rendering main component with', ccis?.length || 0, 'CCIs');
