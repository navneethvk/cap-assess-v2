...
const TimelineCard: React.FC<{ 
  ...
  return (
    ...
    >
      ...
      <Card className={`relative mb-4 transition-all duration-200 bg-white dark:bg-[hsl(var(--card))] border border-blue-100/70 dark:border-foreground/20 rounded-xl shadow-md ${anyDragging ? 'max-h-16 overflow-hidden' : ''}`}>
        <CardHeader 
...
const VisitsTimeline: React.FC = () => {
  ...
  return (
    <div className="mt-4 relative">
      ...
      <div
        className="absolute left-1/2 top-0 bottom-0 -translate-x-1/2 w-14"
        onMouseMove={(e) => {
          const rootRect = (e.currentTarget.parentElement as HTMLElement).getBoundingClientRect()
          const indicator = document.getElementById('spine-add-indicator')
          if (indicator) {
            const yWithin = e.clientY - rootRect.top // relative to the timeline container
        ...
        onClick={(e) => {
          // Compute order relative to current list based on Y position between items
          const positions = Array.from(document.querySelectorAll('[data-visit-card-pos]')) as HTMLElement[]
          const midpoints = positions.map(el => el.getBoundingClientRect().top + el.getBoundingClientRect().height / 2)
          let insertIndex = midpoints.findIndex(m => e.clientY < m)
      ...
      <div id="spine-add-indicator" style={{ display: 'none' }} className="pointer-events-none absolute left-1/2 -translate-x-1/2 z-10">
        <button
          type="button"
          className="pointer-events-auto translate-y-[-50%] h-10 w-10 rounded-full border-2 border-blue-300 text-blue-900 bg-white shadow-sm flex items-center justify-center"
          onClick={(e) => {
            e.stopPropagation()
