...
const UserSettings: React.FC = () => {
  ...
  // swipe across content area to change tabs on mobile
  ...
  const onTouchEnd: React.TouchEventHandler<HTMLDivElement> = (e) => {
    ...
    const dx = endX - start
    const order: ('users'|'ccis'|'assignments')[] = ['users','ccis','assignments']
    const idx = order.indexOf(activeTab)
    const threshold = 40
    const go = (next: 'users'|'ccis'|'assignments') => {
      if (!hasChanges) { setActiveTab(next); return }
      const wantSave = window.confirm('You have unsaved changes. Save before leaving?\nPress OK to Save, Cancel to Stay.')
      if (wantSave) {
        handleSaveChanges().then(() => setActiveTab(next))
      }
      // if cancel, stay on current tab
    }
    if (dx <= -threshold && idx < order.length - 1) go(order[idx+1])
    else if (dx >= threshold && idx > 0) go(order[idx-1])
      ...
      <div className="w-full max-w-7xl mx-auto px-3 sm:px-4 py-4 sm:py-8">
        ...
        {isAdmin ? (
          ...
          <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as any)} className="space-y-4 sm:space-y-6">
            ...
            <TabsContent value="users" className="space-y-6" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
            ...
            <TabsContent value="ccis" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
              <ManageCCIs />
            </TabsContent>

            <TabsContent value="assignments" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
              <AssignUsers />
            </TabsContent>
          </Tabs>
