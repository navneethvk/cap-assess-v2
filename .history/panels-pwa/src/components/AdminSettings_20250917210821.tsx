...
const UserSettings: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'users'|'ccis'|'assignments'|'import'>('users');
    ...
    const checkAdminStatus = async () => {
      if (user && canReadUsers(user)) {
        try {
          ...
          console.log('AdminSettings - Token claims:', idTokenResult.claims);
          console.log('AdminSettings - Role from token:', role);
          console.log('AdminSettings - User UID:', user.uid);
  ...
  return (
    <div className="min-h-screen bg-background pb-20 md:pb-0 m3-expressive">
            ...
            <TabsContent value="users" className="space-y-6">
              <Card className="m3-card">
                ...
                <CardContent className="p-0">
                  {addingUser && (
                    <form className="p-4 border-b bg-muted/30" onSubmit={async (e) => {
                      e.preventDefault();
                      try {
                        // Refresh token to ensure Admin claim is present
                        try { await auth.currentUser?.getIdToken(true) } catch {}
                        // Some extensions block 'admin*' endpoints; use alternate name
                        const fn = httpsCallable(getFunctions(app, 'us-central1'), 'panelCreateUser');
                        const res: any = await fn({ email: newEmail, password: newPassword, role: newRole, username: newUsername });
                        console.log('adminCreateUser result', res?.data);
                        ...
                        if (code.includes('permission-denied')) {
                          notify.error('Admin privileges required. Try reloading to refresh your token.');
                        } else if (code.includes('unauthenticated')) {
                          notify.error('Please sign in again to create users.');
                        } else if (code.includes('invalid-argument')) {
                          notify.error('Invalid input. Ensure email and password (>=6 chars) are set.');
                        } else {
                          notify.error(e?.message || 'Failed to create user');
                        }
                      }
                    }}>
                      <div className="grid grid-cols-1 sm:grid-cols-4 gap-3 items-end">
                        <div>
                          ...
                          <input className="mt-1 w-full rounded-md border px-3 py-2 text-sm" value={newUsername} onChange={(e) => setNewUsername(e.target.value)} placeholder="Full name / Display name" />
                        </div>
                        <div>
                          <label className="text-xs text-muted-foreground">Email</label>
                          <input className="mt-1 w-full rounded-md border px-3 py-2 text-sm" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} placeholder="user@example.com" />
                        </div>
                        <div>
                          <label className="text-xs text-muted-foreground">Password</label>
                          <input name="admin-new-password" className="mt-1 w-full rounded-md border px-3 py-2 text-sm" type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} placeholder="At least 6 characters" />
                        </div>
                        <div>
                          ...
                          <select className="mt-1 w-full rounded-md border px-3 py-2 text-sm" value={newRole} onChange={(e) => setNewRole(e.target.value)}>
                            {['Pending','EM','CM','Academy','Management','Guest','Admin'].map(r => (
                              <option key={r} value={r}>{r}</option>
                            ))}
                          </select>
                        </div>
                        <div className="flex gap-2 justify-end">
                          <Button type="button" variant="ghost" onClick={() => { setAddingUser(false); setNewEmail(''); setNewPassword(''); setNewUsername(''); }}>Cancel</Button>
            ...
            <TabsContent value="snapshots">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Create snapshots for existing version history events. This will group events into versioned snapshots for better organization.
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="space-y-2">
                      <h4 className="font-medium">Process All Visits</h4>
                      <p className="text-sm text-muted-foreground">
                        Create snapshots for all visits that have version history events.
                      </p>
                      <Button 
                        onClick={handleCreateSnapshotsAll}
                        disabled={isCreatingSnapshots}
                        className="w-full"
                      >
                        {isCreatingSnapshots ? 'Processing...' : 'Create Snapshots for All Visits'}
                      </Button>
                    </div>
                    <div className="space-y-2">
                      <h4 className="font-medium">Process Specific Visit</h4>
                      <p className="text-sm text-muted-foreground">
                        Create snapshots for a specific visit by ID.
                      </p>
                      <div className="flex gap-2">
                        ...
                        />
                        <Button 
                          onClick={handleCreateSnapshotsForVisit}
                          disabled={isCreatingSnapshots || !visitIdInput.trim()}
                          size="sm"
                        >
                          {isCreatingSnapshots ? 'Processing...' : 'Create'}
                        </Button>
                      </div>
                    </div>
                  </div>
                  ...
                  {snapshotResult && (
                    <div className="p-4 bg-green-50 border border-green-200 rounded-md">
                      ...
                      <p className="text-sm text-green-700">{snapshotResult.message}</p>
                      {snapshotResult.snapshotsCreated && (
                        <p className="text-sm text-green-700">
                          Created {snapshotResult.snapshotsCreated} snapshots
                        </p>
                      )}
                      {snapshotResult.totalVisitsProcessed && (
                        <p className="text-sm text-green-700">
                          Processed {snapshotResult.totalVisitsProcessed} visits
                        </p>
                      )}
                    </div>
                  )}
