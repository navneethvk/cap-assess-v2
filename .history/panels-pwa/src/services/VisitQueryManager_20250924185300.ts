...
class VisitQueryManager {
  ...
  private applyPermissions(options: VisitQueryOptions): VisitQueryOptions {
    ...
    // NEW LOGIC: Check See All setting
    // If See All is enabled, show all visits (no filtering by user)
    if (options._seeAllVisits === true) {
      return options;
    }

    // For non-admin users with See All disabled, filter by their UID
