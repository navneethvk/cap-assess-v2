...
class VisitQueryManager {
   ...
   * Note: With updated security rules, most filtering is handled by Firestore
   */
  private async applySeeAllFiltering(result: VisitQueryResult, options: VisitQueryOptions): Promise<VisitQueryResult> {
    ...
    // With the updated security rules, users can now see:
    // - Their own visits
    // - Visits for CCIs they are assigned to
    // The See All toggle is handled by the security rules automatically
    // We only need to do additional filtering if See All is disabled
    if (options._seeAllVisits === false) {
      try {
        ...
        // Filter to only show visits for CCIs the user is assigned to
    ...
    // See All is enabled - return all results (security rules handle permissions)
    return result;
  }
  ...
  /**
   * Fetch visits for specific CCIs
   */
  private async fetchVisitsForCCIs(cciIds: string[], options: VisitQueryOptions): Promise<DocumentWithId<VisitDoc>[]> {
    const { collection, query, where, getDocs } = await import('firebase/firestore');
    const { db } = await import('@/firebase');
    
    const allVisits: DocumentWithId<VisitDoc>[] = [];
    
    // Fetch visits for each CCI (we need to do this one by one due to Firestore limitations)
    for (const cciId of cciIds) {
      try {
        const constraints: any[] = [where('cci_id', '==', cciId)];
        ...
        const visits = snapshot.docs.map(doc => ({
          id: doc.id,
          ...doc.data()
        } as DocumentWithId<VisitDoc>));
        
        allVisits.push(...visits);
      } catch (error) {
        console.warn(`Failed to fetch visits for CCI ${cciId}:`, error);
      }
    }
  ...
  private applyPermissions(options: VisitQueryOptions): VisitQueryOptions {
    ...
    // For non-admin users, the Firestore security rules now handle permissions:
    // - Users can see their own visits (filledByUid == user.uid)
    // - Users can see visits for CCIs they are assigned to (via isEMForCCI function)
    // - The See All toggle is handled by the security rules automatically
    return options;
  }
