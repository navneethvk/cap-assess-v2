...
class VisitQueryManager {
  ...
  private async applySeeAllFiltering(result: VisitQueryResult, options: VisitQueryOptions): Promise<VisitQueryResult> {
    // If See All is enabled or user is admin, return all results
    if (options._seeAllVisits === true || options._isAdmin) {
      return result;
    }

    // If See All is disabled, filter to only show visits for CCIs the user is assigned to
    if (!options._user) {
      return result; // No user context, return as-is
    }
    ...
    try {
      ...
      // Filter visits to only include those for CCIs the user is assigned to
  ...
  private applyPermissions(options: VisitQueryOptions): VisitQueryOptions {
    ...
    // For non-admin users, the Firestore security rules will handle permissions:
    // - Users can always see their own visits (filledByUid == user.uid)
    // - Users can see visits for CCIs they are assigned to (via isEMForCCI function)
    // - The See All toggle will be handled at the application level after fetching
    //   by filtering the results based on the user's CCI assignments
    return options;
  }
  ...
  async queryVisits(options: VisitQueryOptions = {}): Promise<VisitQueryResult> {
    ...
    // Apply See All filtering at application level
    const filteredResult = this.applySeeAllFiltering(result, options);
