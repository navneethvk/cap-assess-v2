...
export const usePermissionsStore = create<PermissionsState>((set, get) => ({
  ...
  canEditVisit: (visitId: string, filledByUid: string) => {
    ...
    // Also allow EMs to edit their own visits even if userId is not set yet (fallback)
    const canEdit = capabilities.canEditVisits && (
      userId === filledByUid || 
      (state.isEM && !userId) // Fallback for EMs when userId is not yet set
    );
