    ...
    (set, get) => ({
      ...
      clearStore: () => {
        ...
        if (typeof window !== 'undefined') {
          ...
          try {
            localStorage.removeItem('visit-store');
          } catch {}
          try {
            // Clear SWR cache if available
            const swrCache = (window as any).__SWR_DEVTOOLS_RESTORE__?.cache || (window as any).__SWR_CONFIG__?.cache
            if (swrCache && swrCache.clear) swrCache.clear()
          } catch {}
        }
      },
      refreshToken: async () => {
