...
async function checkInsightsCollectionExists(): Promise<boolean> {
  try {
    ...
    logger.error('Error checking insights collection:', error);
    return false;
  }
}
...
export const insightDataAggregator = onSchedule({
  ...
  try {
    ...
    if (!insightsExist) {
      logger.info('Insights collection does not exist. Running initial aggregation for historical data...');
      ...
      for (const date of datesToAggregate) {
        try {
          ...
          logger.warn(`Failed to aggregate data for date ${date.toISOString()}:`, error);
          // Continue with other dates even if one fails
        }
      }
      
      logger.info(`Initial aggregation completed for ${datesToAggregate.length} weeks`);
    } else {
      logger.info('Insights collection exists. Running daily aggregation...');
...
/**
 * Manual trigger function for testing or one-time execution
 */
export const triggerInsightAggregation = functions.https.onCall(async (data, context) => {
  ...
  if (role !== 'Admin') {
    throw new functions.https.HttpsError('permission-denied', 'Admin privileges required to trigger aggregation');
  }

  try {
    logger.info('Manual insight aggregation triggered by admin', {
    ...
    if (!insightsExist) {
      logger.info('Insights collection does not exist. Running initial aggregation...');
      ...
      return {
        success: true,
        message: `Initial aggregation completed for ${datesToAggregate.length} weeks`,
        weeksProcessed: datesToAggregate.length
      };
    } else {
      logger.info('Insights collection exists. Running daily aggregation...');
      
      // Regular daily aggregation
      const targetDate = new Date();
      targetDate.setDate(targetDate.getDate() - 1);
      await aggregateWeeklyVisitsCount(targetDate);
    ...
    logger.error('Manual insight aggregation failed:', error);
    throw new functions.https.HttpsError('internal', `Failed to trigger aggregation: ${error}`);
  }
});
