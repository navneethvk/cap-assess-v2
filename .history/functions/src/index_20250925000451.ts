...
async function aggregateWeeklyVisitsCount(targetDate: Date): Promise<void> {
  try {
    ...
    userStats.forEach((user, uid) => {
      const userKey = user.email || user.username || uid; // Use email as primary key, fallback to username or UID
      
      perUserStats.set(userKey, {
        uid: user.uid,
        email: user.email,
    ...
    const userBreakdown = {
      ...
      userStats: Object.fromEntries(
        Array.from(userStats.entries()).map(([uid, user]) => [
          uid,
          {
            email: user.email,
            username: user.username,
