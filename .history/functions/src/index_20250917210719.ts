...
async function checkAndCreateSnapshot(visitId: string) {
  try {
    ...
    // Get all events that are not part of a snapshot
    ...
    // If we have 10 or more events, create a snapshot
    if (events.length >= 10) {
      ...
      const latestVersion = snapshotsSnapshot.empty ? 0 : snapshotsSnapshot.docs[0].data().version;
      ...
      const snapshotData = {
        ...
        events: events.slice(0, 10), // Take the first 10 events
        createdAt: admin.firestore.FieldValue.serverTimestamp(),
        eventCount: 10,
        summary: generateSnapshotSummary(events.slice(0, 10))
      };
      ...
      // Mark the first 10 events as part of this snapshot
      const batch = firestore.batch();
      for (let i = 0; i < 10; i++) {
        const eventRef = eventsRef.doc(events[i].id);
...
// Cloud Function to retrospectively create snapshots for existing events
export const createRetrospectiveSnapshots = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Authentication required.");
  }
  ...
  const visitId = data?.visitId as string;
  if (!visitId) {
  ...
  try {
    ...
    // Get all events that are not part of a snapshot, ordered by timestamp
    const eventsSnapshot = await eventsRef
      .where("isSnapshot", "==", false)
      .orderBy("timestamp", "asc")
      .get();
    ...
    if (events.length === 0) {
      return { message: "No events to process", snapshotsCreated: 0 };
    }
    ...
    for (const batch of batches) {
      ...
      // Mark all events in this batch as part of this snapshot
      ...
      functions.logger.info(`Created retrospective snapshot Version ${newVersion} for visit ${visitId}`, {
        visitId,
        version: newVersion,
        eventCount: batch.length
      });
    ...
    return { 
      message: `Successfully created ${snapshotsCreated} snapshots for visit ${visitId}`,
      snapshotsCreated,
      totalEventsProcessed: events.length
    };
    
  } catch (error) {
    functions.logger.error(`Error creating retrospective snapshots for visit ${visitId}:`, error);
    throw new functions.https.HttpsError("internal", `Failed to create retrospective snapshots: ${error}`);
  }
});

// Cloud Function to create retrospective snapshots for ALL visits
export const createRetrospectiveSnapshotsAll = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "Authentication required.");
  }
  ...
  try {
    ...
    for (const visit of visits) {
      try {
        ...
        if (events.length === 0) {
          continue; // Skip visits with no events
        }
        ...
        // Process each batch for this visit
        for (const batch of batches) {
          // Get the latest snapshot number for this visit
          const snapshotsSnapshot = await firestore
            .collection("visits")
        ...
        functions.logger.error(`Error processing visit ${visit.id}:`, error);
        // Continue with other visits even if one fails
      }
    }
    
    return { 
      message: `Successfully processed ${totalVisitsProcessed} visits and created ${totalSnapshotsCreated} snapshots`,
      totalSnapshotsCreated,
      totalVisitsProcessed
    };
    
  } catch (error) {
    functions.logger.error("Error creating retrospective snapshots for all visits:", error);
    throw new functions.https.HttpsError("internal", `Failed to create retrospective snapshots: ${error}`);
  }
});
